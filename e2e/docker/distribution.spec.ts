...
const secret =
  ...
  "docker-verification-secret-never-exposed";
...
async function setFakeScenario(
  request: APIRequestContext,
  scenario: "healthy" | "timeout",
): Promise<void> {
  if (!fakeSpeachesUrl)
    throw new Error("STUDYNARRATOR_FAKE_SPEACHES_URL is required.");
  await jsonRequest(request, "put", `${fakeSpeachesUrl}/__control/scenario`, {
    scenario,
  });
}
...
async function pollRender(
  ...
  while (Date.now() < deadline) {
    const render = (await jsonRequest(
      request,
      "get",
      `/api/renders/${renderId}`,
    )) as Record<string, unknown>;
    if (["complete", "failed", "canceled"].includes(String(render.state)))
      return render;
...
test("Docker Web remains authorable offline and renders after Speaches reconnects", async ({
  browserName,
  page,
  request,
}) => {
  const runtime = (await jsonRequest(request, "get", "/api/runtime")) as Record<
    string,
    unknown
  >;
  ...
  expect(runtime.sourceRevision).toBe(
    process.env.STUDYNARRATOR_EXPECTED_SOURCE_REVISION,
  );

  const initialConnection = (await jsonRequest(
    request,
    "get",
    "/api/connection",
  )) as Record<string, unknown>;
  expect(
    fakeSpeachesAppUrl,
    "STUDYNARRATOR_FAKE_SPEACHES_APP_URL is required.",
  ).toBeTruthy();
  ...
  if (browserName === "chromium") {
    ...
    const catalog = (await jsonRequest(
      ...
      models: Array<{ modelId: string; voices: Array<{ voiceId: string }> }>;
    };
  ...
  const initialTest = (await jsonRequest(
    request,
    "post",
    "/api/connection/test",
  )) as Record<string, unknown>;
  ...
  const created = (await jsonRequest(request, "post", "/api/projects", {
    name,
    description: "Created while Speaches is offline.",
  })) as {
    id: string;
    name: string;
  };
  ...
  const timing = (await jsonRequest(
    request,
    "get",
    "/api/settings/pacing",
  )) as {
  ...
  await jsonRequest(request, "put", "/api/settings/pacing", {
    ...
    transitionPauses: {
      ...
      paragraph: { mode: "duration", durationMs: 625 },
    },
  });
  ...
  await jsonRequest(request, "put", `/api/projects/${created.id}`, {
    name,
    description:
      "Created offline and rendered after reconnecting without a container restart.",
  ...
  const started = (await jsonRequest(
    request,
    "post",
    `/api/projects/${created.id}/renders`,
  )) as { id: string };
  const renders = (await jsonRequest(
    request,
    "get",
    `/api/projects/${created.id}/renders`,
  )) as Array<{ id: string }>;
  ...
  await expect(
    page.getByText(String(runtime.sourceRevision), { exact: true }),
  ).toBeVisible();
  ...
  expect(
    JSON.stringify({ runtime, diagnostics, connectionDiagnostics, connection }),
