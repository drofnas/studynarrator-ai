...
class ElectronStudyNarrator {
  ...
  static async start(
    webApplication: StudyNarratorTestApplication,
  ): Promise<ElectronStudyNarrator> {
    ...
    return new ElectronStudyNarrator(
      application,
      page,
      dataDirectory,
      webApplication,
...
export const test = studyNarratorTest.extend<{
  electronStudyNarrator: ElectronStudyNarrator;
}>({
...
export async function configureElectronConnection(
  page: Page,
  application: StudyNarratorTestApplication,
): Promise<void> {
  const onboarding = page.getByRole("heading", {
