...
import {
  configureElectronConnection,
  continueElectronOffline,
  expect,
  test,
...
async function openProject(page: Page, name: string): Promise<void> {
  await page
    .getByRole("row", { name: new RegExp(name, "u") })
    .getByRole("link", { name })
    .click();
  await expect(
    page.getByRole("tab", { name: "Script Editor" }),
...
test.describe("Electron acceptance", () => {
  test("uses typed IPC for Scratchpad playback and exposes no Node or generic primitive", async ({
    electronStudyNarrator,
    studyNarrator,
  }) => {
    ...
    const bridgeShape = await page.evaluate(() => {
      ...
      return {
        ...
        frozen: renderer.studyNarrator
          ? Object.isFrozen(renderer.studyNarrator)
          : false,
      };
    });
    ...
    await page
      .getByLabel("Passage")
      .fill("SQL indexes can improve database reads.");
      ...
      .poll(
        () =>
          studyNarrator.fakeSpeaches
            .getState()
            .requests.filter(({ path }) => path === "/v1/audio/speech").length,
      )
      .toBe(1);
    ...
    expect(
      studyNarrator.fakeSpeaches
        .getState()
        .requests.filter(({ path }) => path === "/v1/audio/speech"),
    ).toHaveLength(1);
    await renderButton.click();
    await expect(renderButton).toBeEnabled();
    expect(
    ...
    await page
      ...
      .fill("[speaker_narrator] Globally configured model voice.");
    await page.getByRole("tab", { name: "Settings" }).click();
    await expect(page.getByLabel("Optional model override")).toHaveCount(0);
    await expect(page.getByLabel("Voice for speaker narrator")).toHaveValue(
      "af_heart",
    );
    await expect(
      page
        .getByLabel("Voice for speaker narrator")
        .getByRole("option", { name: "Heart (af_heart | en-US)" }),
    ).toBeAttached();
    await expect(
      page
  ...
  test("persists project UI state across a desktop relaunch", async ({
    electronStudyNarrator,
  }) => {
    ...
    await page
      .getByLabel("Script source")
      .fill("[speaker_teacher] Persist through relaunch.");
    await page.getByRole("tab", { name: "Settings" }).click();
    ...
    await expect(
      lexicon.getByRole("article", { name: "Lexicon entry CLI" }),
    ).toBeVisible();
    await page.getByRole("button", { name: "Save now" }).click();
    await expect(page.getByText("All changes saved.")).toHaveCount(0);
    ...
    await expect(page.getByLabel("Script source")).toHaveText(
      "[speaker_teacher] Persist through relaunch.",
    );
    await page.getByRole("tab", { name: "Settings" }).click();
    await expect(
      page
        .getByRole("region", { name: "Project lexicon" })
        .getByRole("article", { name: "Lexicon entry CLI" }),
    ).toBeVisible();
  });

  test("keeps one hidden current plan and reopens the latest render through typed IPC", async ({
    electronStudyNarrator,
    studyNarrator,
  }) => {
    let page = electronStudyNarrator.page;
    ...
    if (!projectId)
      throw new Error("Project route did not contain a project ID.");
    const firstRenders = await page.evaluate(
      async (id) =>
        await (
          window as typeof window & { studyNarrator: StudyNarratorBridge }
        ).studyNarrator.renders.list(id),
      projectId,
    );
    ...
    const currentRenders = await page.evaluate(
      async (id) =>
        await (
          window as typeof window & { studyNarrator: StudyNarratorBridge }
        ).studyNarrator.renders.list(id),
  ...
  test("auto-resumes an interrupted render and saves a validated artifact through native IPC", async ({
    ...
    await page
      .getByLabel("Script source")
      .fill("[speaker_teacher] Resume this render.");
    await page.getByRole("tab", { name: "Settings" }).click();
    await expect(page.getByLabel("Optional model override")).toHaveCount(0);
    await expect(page.getByLabel("Voice for speaker teacher")).toHaveValue(
      "af_heart",
    ...
    await electronStudyNarrator.application.evaluate(
      ({ dialog }: ElectronEvaluationApi, filePath) => {
  ...
  test("edits and saves independent prompts through native typed IPC without TTS", async ({
    ...
    const creationDestination = resolve(
      dataDirectory,
      "desktop-creation-prompt.md",
    );
    const updateDestination = resolve(
      dataDirectory,
      "desktop-update-prompt.md",
    );
    await application.evaluate(
      ({ dialog }: ElectronEvaluationApi, destinations) => {
        dialog.showSaveDialog = ({ defaultPath }) =>
          Promise.resolve({
            canceled: false,
            filePath: defaultPath.includes("update")
              ? destinations.update
    ...
    await page
      ...
      .click();
    await expect
      .poll(async () => (await stat(creationDestination)).size)
      .toBeGreaterThan(0);
    ...
    await expect
      .poll(async () => (await stat(updateDestination)).size)
      .toBeGreaterThan(0);
    await expect(
  ...
  test("exposes and persists only the singleton connection settings", async ({
    electronStudyNarrator,
    studyNarrator,
  }) => {
    let page = electronStudyNarrator.page;
  ...
  test("opens only approved Speaches links outside the renderer", async ({
    ...
    await page
      .getByRole("link", { name: "Official installation guide" })
      .click();
    await expect
      .poll(
        async () =>
    ...
    await page.evaluate(() =>
      window.open("https://example.com/not-approved", "_blank"),
    );
    await expect
      .poll(
        async () =>
