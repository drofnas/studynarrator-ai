...
test.describe("render execution", () => {
  ...
  test("keeps one render per project while repeated renders reuse cached speech", async ({ page, request, studyNarrator }) => {
    await configureConnection(page, studyNarrator);
    const created = await (await request.post(`${studyNarrator.baseUrl}/api/projects`, { data: { name: "One render per project" } })).json() as { id: string; name: string };
    await request.put(`${studyNarrator.baseUrl}/api/projects/${created.id}`, { data: {
      name: created.name, description: "One render per project.", scriptSource: "[speaker_teacher] Reuse this narration.",
    ...
    const firstRenders = await (await request.get(`${studyNarrator.baseUrl}/api/projects/${created.id}/renders`)).json() as Array<{ id: string }>;
    ...
    await page.getByRole("button", { name: "Render" }).click();
    await expect(page.getByRole("button", { name: "Download", exact: true })).toBeVisible({ timeout: 20_000 });
    const currentRenders = await (await request.get(`${studyNarrator.baseUrl}/api/projects/${created.id}/renders`)).json() as Array<{ id: string }>;
