...
describe("fake Speaches diagnostic scenarios", () => {
  it("can bind on all interfaces for Docker host-gateway acceptance", async () => {
    ...
    expect((await fetch(`${current.baseUrl}/__control/state`)).status).toBe(
      200,
    );
  });
  ...
  it("reports disjoint model-scoped voice lists", async () => {
    ...
    expect(
      catalog.models
        .find(({ modelId }) => modelId === FAKE_SPEACHES_MODEL_ID)
        ?.voices.map(({ voiceId }) => voiceId),
    ).toEqual(["af_heart", "af_sky"]);
    expect(
      catalog.models
        .find(({ modelId }) => modelId === FAKE_SPEACHES_SECONDARY_MODEL_ID)
        ?.voices.map(({ voiceId }) => voiceId),
    ).toEqual([FAKE_SPEACHES_SECONDARY_VOICE_ID]);
  });
  ...
  it("returns deterministic, decodable WAV audio and sanitized logs", async () => {
    ...
    expect(output.summary.stages.map(({ status }) => status)).toEqual(
      Array(8).fill("pass"),
    );
  ...
  it.each([
    ...
    expect(
      output.summary.stages.some((candidate) => candidate.code === code),
    ).toBe(true);
  ...
  it("times out once with no retry", async () => {
    ...
    expect(current.getState().requests).toEqual([
      expect.objectContaining({ method: "GET", path: "/health", status: 0 }),
    ]);
  });
  ...
  it("accepts root and /v1 inputs without ever requesting /v1/v1", async () => {
    ...
    expect(
      Object.keys(state.counters).some((path) => path.includes("/v1/v1")),
    ).toBe(false);
  });
  ...
  it("changes scenarios and clears counters through loopback controls", async () => {
    ...
    expect((await diagnose(current.baseUrl)).summary.overall).toBe(
      "modelUnavailable",
    );
    const reset = await fetch(`${current.baseUrl}/__control/reset`, {
      method: "DELETE",
    });
