...
describe("SharedAudioPlayer", () => {
  it("provides full transport, volume, mute, and stop controls", async () => {
    const { container } = render(<SharedAudioPlayer label="Chapter one" src="/chapter.mp3" waveform={waveform} />);
    ...
    await act(async () => { fireEvent.click(screen.getByRole("button", { name: "Replay" })); await Promise.resolve(); });
    expect(screen.getByRole("status")).toHaveTextContent("Playing");
  });
  ...
  it("seeks through keyboard and waveform pointer input", () => {
    ...
    vi.spyOn(signal, "getBoundingClientRect").mockReturnValue({ x: 10, y: 0, left: 10, top: 0, width: 200, height: 54, right: 210, bottom: 54, toJSON: () => ({}) });
    fireEvent.pointerDown(signal, { clientX: 60 });
    expect(audio.currentTime).toBe(5);
  });

  it("uses fallback progress, resets on a source change, and reports media failure", () => {
    const { container, rerender } = render(<SharedAudioPlayer label="Preview" src="/one.wav" waveform={{ status: "unavailable", renderId: waveform.renderId, reason: "extractionFailed" }} />);
    const audio = loadAudio(container, 12);
    expect(screen.getByRole("group", { name: "Playback progress" })).toBeInTheDocument();
    fireEvent.change(screen.getByLabelText("Seek playback"), { target: { value: "7" } });
