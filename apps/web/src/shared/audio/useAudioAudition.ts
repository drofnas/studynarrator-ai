...
export function useAudioAudition<Key extends string | number>() {
  const [audition, setAudition] = useState<AudioAuditionState<Key> | null>(null);
  ...
  const stop = useCallback((resetState = true) => {
    ...
    if (sourceRef.current) {
      ...
      try { sourceRef.current.stop(); } catch { /* The source may not have started yet. */ }
  ...
  const play = useCallback(async (key: Key, load: (signal: AbortSignal) => Promise<AuditionAudio>) => {
    stop();
    ...
    try {
      ...
      if (controller.signal.aborted || generation !== generationRef.current) return;
      const buffer = await context.decodeAudioData(decodedAudio(audio.base64));
      if (controller.signal.aborted || generation !== generationRef.current) return;
