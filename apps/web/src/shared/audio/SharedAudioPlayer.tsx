...
type PlaybackState =
  "loading" | "ready" | "playing" | "paused" | "complete" | "unavailable";
...
export function SharedAudioPlayer({
  ...
  const effectiveDuration =
    duration || (availableWaveform?.durationMs ?? 0) / 1_000;
  const waveformPath = useMemo(() => {
    ...
    const bottom = [...availableWaveform.peaks]
      .reverse()
  ...
  return (
    ...
    >
      <audio
        ...
        onEnded={() => {
          setCurrentTime(audioRef.current?.duration ?? effectiveDuration);
          setState("complete");
        }}
        onError={() => setState("unavailable")}
      />
      <div className={styles.identity}>
        <span>Loaded audio</span>
        <strong>{label}</strong>
      </div>
      <div className={styles.transport}>
        ...
        <button
          ...
          onClick={stop}
          disabled={disabled || state === "loading" || state === "unavailable"}
        >
          Stop
        </button>
        <button
      ...
      <div
        ...
        role="group"
        aria-label={
          availableWaveform ? "Playback waveform" : "Playback progress"
        }
      ...
      >
        {availableWaveform ? (
          ...
          <span className={styles.fallback}>
            <i
              style={{
                width: `${String(effectiveDuration > 0 ? Math.min(100, (currentTime / effectiveDuration) * 100) : 0)}%`,
              }}
            />
          </span>
        )}
