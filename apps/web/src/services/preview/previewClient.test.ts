...
describe("preview REST clients", () => {
  it("posts project previews with cancellation and validates the response", async () => {
    ...
    await expect(
      createRestProjectPreviewClient(fetchInput as typeof fetch).preview(
        preview.projectId,
    ...
    expect(fetchInput).toHaveBeenCalledWith(
      `/api/projects/${preview.projectId}/preview`,
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ mode: "pronunciation", text: "Speech." }),
  ...
  it("prefers both Electron preload clients", () => {
    ...
    const browser = {
      studyNarrator: { projectPreview, speechCache },
