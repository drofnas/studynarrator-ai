...
export function createRestPersistenceClient(
  ...
  return {
    status: async () =>
      await request("/api/persistence/status", PersistenceStatusSchema),
    backups: {
      list: async () =>
        await request(
    ...
    projects: {
      list: async () =>
        await request("/api/projects", ProjectSummaryCollectionSchema),
      create: async (input) =>
        await request("/api/projects", ProjectDetailSchema, {
          method: "POST",
          body: body(input),
        }),
      get: async (projectId) =>
      ...
      replace: async (projectId, input) =>
        await request(
          `/api/projects/${encodeURIComponent(projectId)}`,
          ProjectDetailSchema,
          { method: "PUT", body: body(input) },
        ),
      duplicate: async (projectId, input) =>
        await request(
    ...
    settings: {
      getPacing: async () =>
        await request("/api/settings/pacing", SystemTimingConfigurationSchema),
      updatePacing: async (input) =>
        await request("/api/settings/pacing", SystemTimingConfigurationSchema, {
          method: "PUT",
          body: body(input),
        }),
    ...
    preferences: {
      ...
      replaceIgnoredDiagnostics: async (input) =>
        await request(
          "/api/preferences/ignored-diagnostics",
          IgnoredDiagnosticCollectionSchema,
          { method: "PUT", body: body(input) },
...
export function resolvePersistenceClient(
  browserWindow: Window = window,
): PersistenceClient {
  return (
    browserWindow.studyNarrator?.persistence ?? createRestPersistenceClient()
  );
}
