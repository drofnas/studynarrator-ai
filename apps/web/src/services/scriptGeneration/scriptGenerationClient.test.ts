...
describe("REST script generation client", () => {
  ...
  it("downloads prompt and skill exports with server filenames", async () => {
    ...
    await expect(client.exportPrompt(null, "update", "Edited prompt")).resolves.toEqual({ disposition: "download", fileName: "prompt.md" });
    await expect(client.exportSkillPackage(null)).resolves.toEqual({ disposition: "download", fileName: "skill.zip" });
    expect(click).toHaveBeenCalledTimes(2);
    expect(fetchMock).toHaveBeenCalledWith("/api/script-generation/prompt-export", expect.objectContaining({ body: JSON.stringify({ kind: "update", content: "Edited prompt" }) }));
  });
  ...
  it("keeps generated prompt export compatible when no content override is supplied", async () => {
    ...
    await createRestScriptGenerationClient(fetchMock).exportPrompt(null, "creation");
    expect(fetchMock).toHaveBeenCalledWith("/api/script-generation/prompt-export", expect.objectContaining({ body: JSON.stringify({ kind: "creation" }) }));
  });
