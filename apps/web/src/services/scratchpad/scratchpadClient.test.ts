...
import {
  createRestScratchpadClient,
  resolveScratchpadClient,
} from "./scratchpadClient.js";
...
describe("Scratchpad client", () => {
  it("posts the validated request and forwards cancellation", async () => {
    ...
    await expect(
      createRestScratchpadClient(fetchInput as typeof fetch).preview(
        input,
        controller.signal,
      ),
  ...
  it("uses sanitized boundary errors and rejects malformed success output", async () => {
    ...
    await expect(
      createRestScratchpadClient(failure as typeof fetch).preview({
        modelId: "m",
        voiceId: "v",
        speed: 1,
        text: "x",
    ...
    const malformed = vi.fn(
      async () =>
        new Response(JSON.stringify({ ...result, secret: true }), {
          status: 200,
        }),
    );
    await expect(
      createRestScratchpadClient(malformed as typeof fetch).preview({
        modelId: "m",
        voiceId: "v",
        speed: 1,
        text: "x",
