...
describe("Scratchpad client", () => {
  it("posts the validated request and forwards cancellation", async () => {
    ...
    const input = { connectionProfileId: "profile", modelId: "model", voiceId: "voice", speed: 1, text: "Speech.", applyGlobalLexicon: false };
  ...
  it("uses sanitized boundary errors and rejects malformed success output", async () => {
    ...
    await expect(createRestScratchpadClient(failure as typeof fetch).preview({ connectionProfileId: "p", modelId: "m", voiceId: "v", speed: 1, text: "x", applyGlobalLexicon: false }))
    ...
    await expect(createRestScratchpadClient(malformed as typeof fetch).preview({ connectionProfileId: "p", modelId: "m", voiceId: "v", speed: 1, text: "x", applyGlobalLexicon: false })).rejects.toThrow();
  });
