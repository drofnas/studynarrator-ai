...
import { createRestConnectionClient, createRestVoiceCatalogClient, resolveConnectionClient, resolveVoiceCatalogClient } from "./connectionsClient.js";
...
const connection = {
  ...
  responseFormat: "wav", lastTestedAt: null, lastSuccessfulTestAt: null, lastTestSummary: null,
...
describe("connection REST clients", () => {
  it("prefers the narrow Electron clients", () => {
    ...
    const browser = { studyNarrator: { connection: electronConnection, voiceCatalog } } as never;
  ...
  it("updates the singular connection route", async () => {
    const fetchInput = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => new Response(JSON.stringify(connection), { status: 200 }));
    await createRestConnectionClient(fetchInput).update({
      baseUrl: "http://127.0.0.1:8000", defaultModelId: "model", defaultVoiceId: "voice",
      timeoutSeconds: 120, retryCount: 2, responseFormat: "wav"
    });
  ...
  it("discovers a draft speech catalog without persisting it", async () => {
    const catalog = { schemaVersion: 1, models: [{ modelId: "model", voices: [{ voiceId: "voice", name: null, language: null, gender: null }] }] };
    const fetchInput = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => new Response(JSON.stringify(catalog), { status: 200 }));
    const draft = { baseUrl: "http://127.0.0.1:8000", timeoutSeconds: 120, retryCount: 2 };
    await expect(createRestConnectionClient(fetchInput).discoverSpeechCatalog(draft)).resolves.toEqual(catalog);
    expect(fetchInput.mock.calls[0]?.[0]).toBe("/api/connection/speech-catalog");
    expect(fetchInput.mock.calls[0]?.[1]).toMatchObject({ method: "POST", body: JSON.stringify(draft) });
  });

  it("rejects malformed connection output", async () => {
    const fetchInput = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => new Response(JSON.stringify({ ...connection, apiKey: "test-secret-must-not-appear" }), { status: 200 }));
    await expect(createRestConnectionClient(fetchInput).get()).rejects.toThrow();
  });
