...
function createRequest(fetchInput: typeof fetch) {
  return async <T>(path: string, schema: RuntimeSchema<T>, init?: RequestInit): Promise<T> => {
    ...
    if (!response.ok) {
      ...
      throw new ConnectionClientError(boundary?.error.message ?? "The connection service returned an invalid response.", boundary?.error.code ?? "INVALID_BOUNDARY_RESPONSE", response.status);
    }
    return schema.parse(await response.json());
  };
}

export function createRestConnectionClient(fetchInput: typeof fetch = fetch): SpeachesConnectionClient {
  const request = createRequest(fetchInput);
  return {
    get: async () => await request("/api/connection", SpeachesConnectionSchema),
    update: async (input) => await request("/api/connection", SpeachesConnectionSchema, { method: "PUT", body: JSON.stringify(input) }),
    test: async () => await request("/api/connection/test", ConnectionTestSummarySchema, { method: "POST" }),
    discoverSpeechCatalog: async (input, signal) => await request("/api/connection/speech-catalog", SpeechCatalogSchema, {
      method: "POST", body: JSON.stringify(input), ...(signal === undefined ? {} : { signal })
    }),
    exportDiagnostics: async () => await request("/api/connection/diagnostics", RedactedConnectionDiagnosticsSchema),
...
export function resolveConnectionClient(browserWindow: Window = window): SpeachesConnectionClient {
  return browserWindow.studyNarrator?.connection ?? createRestConnectionClient();
}
