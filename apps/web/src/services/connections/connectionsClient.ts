...
export function createRestConnectionsClient(fetchInput: typeof fetch = fetch): ConnectionsClient {
  ...
  return {
    ...
    discoverSpeechCatalog: async (profileId, signal) => await request(`/api/connections/${encodeURIComponent(profileId)}/speech-catalog`, SpeechCatalogSchema, signal === undefined ? undefined : { signal }),
