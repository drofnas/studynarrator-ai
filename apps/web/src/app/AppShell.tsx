...
export function AppShell() {
  ...
  const promptActive =
    location.pathname === APP_PATHS.scriptPrompts ||
    /\/projects\/[^/]+\/script-generation$/u.test(location.pathname);
  const projectsActive =
    location.pathname.startsWith(APP_PATHS.projects) && !promptActive;
  const onboardingComplete = connections.setup?.onboardingCompletedAt != null;
  const connectionPath = onboardingComplete
    ? APP_PATHS.settingsGeneral
    : APP_PATHS.onboarding;
  const connectionLabel = connections.loading
    ? "Checking connection"
    : connectionLabels[connections.shellState];
  const connectionState = connections.loading
    ? "loading"
    : connections.shellState;
    ...
    <div className={styles.shell}>
      ...
      >
        ...
        <nav className={styles.navigation} aria-label="StudyNarrator tools">
          ...
          <div className={styles.navLinks}>
            ...
            {navigationLink(
              ...
              "Quick Scratchpad",
              "scratchpad",
              location.pathname === APP_PATHS.scratchpad,
            )}
            <div className={styles.settingsGroup}>
              <Link className={styles.navItem} to={APP_PATHS.settingsGeneral}>
                <Icon name="settings" />
                <span>Settings</span>
              </Link>
              <div
                className={styles.settingsSubnav}
            ...
            {navigationLink(
              APP_PATHS.diagnostics,
              "System diagnostics",
              "diagnostics",
              location.pathname === APP_PATHS.diagnostics,
            )}
          </div>
        </nav>
        ...
        <div className={styles.sidebarFooter}>
          <Link
            className={styles.connectionMonitor}
            data-state={connectionState}
            aria-label={`${connectionLabel}. ${connections.connection?.baseUrl ? new URL(connections.connection.baseUrl).host : "Not configured"}. ${onboardingComplete ? "Manage connection" : "Finish setup"}.`}
            to={connectionPath}
          >
            <span className={styles.connectionHeading}>
              <i aria-hidden="true" />
              <strong>{connectionLabel}</strong>
            </span>
            <span className={styles.connectionProfile}>
              {connections.connection?.baseUrl
                ? new URL(connections.connection.baseUrl).host
                : "Not configured"}
            </span>
            <span className={styles.connectionAction}>
              {onboardingComplete ? "Manage connection" : "Finish setup"}
