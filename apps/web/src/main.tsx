...
createRoot(root).render(
  <StrictMode>
    <HashRouter>
      <App analyzer={createScriptAnalysisWorkerClient()} client={resolveSystemClient()} persistence={resolvePersistenceClient()} connections={resolveConnectionsClient()} voiceCatalog={resolveVoiceCatalogClient()} scratchpad={resolveScratchpadClient()} projectPreview={resolveProjectPreviewClient()} speechCache={resolveSpeechCacheClient()} renderPlans={resolveRenderPlanClient()} renders={resolveRenderClient()} scriptGeneration={resolveScriptGenerationClient()} />
    </HashRouter>
  </StrictMode>
);
