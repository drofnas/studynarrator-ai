...
describe("script analysis worker protocol", () => {
  it("parses and transforms a validated request", () => {
    ...
    const response = handleParserWorkerRequest({ requestId: 4, input: { source: "[speaker_teacher] SQL", entries: [entry], paragraphPause } });
  ...
  it("analyzes a bare 100,000-character script under the system narrator without changing its source", () => {
    ...
    const response = handleParserWorkerRequest({ requestId: 12, input: { source, entries: [], paragraphPause } });
    ...
    expect(response).toMatchObject({
      ...
      result: {
        ...
        transformResult: { source, readableTranscript: source, ttsTranscript: source, synthesisReady: true }
      }
    });
  });
      ...
      result: {
        pacingResult: {
          ...
          audits: [
            ...
            { status: "suppressedByExplicitPause", explicitPauseNodeOrdinals: [4] }
          ]
        }
      }
    });
