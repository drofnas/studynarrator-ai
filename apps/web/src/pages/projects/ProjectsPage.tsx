        ...
        {!draft || !project ? <section className={styles.empty}><h3>Start with a project</h3><p>Create a project or choose one from the rail. Speaches can remain offline throughout this workflow.</p></section> : <>
          <main className={styles.editorColumn}>
            ...
            <section className={styles.projectIdentity}>
              ...
              <div className={styles.actionRow}><button type="button" onClick={() => void saveNow()} disabled={saveState === "saving"}>Save now</button><button type="button" className={styles.secondary} onClick={() => void navigate(`/projects/${project.id}/script-generation`)}>Open script prompt kit</button><button type="button" className={styles.secondary} onClick={() => void duplicateProject()}>Duplicate</button><button type="button" className={styles.secondary} onClick={() => void clearProjectCache()}>Clear project cache</button><button type="button" className={styles.danger} onClick={() => void deleteProject()}>Delete</button></div>
            </section>
