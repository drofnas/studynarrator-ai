  ...
  speakerMappings: [
    {
      ...
      speed: 1,
      gainDb: 0,
      roleDescription: "Explains clearly.",
      sampleText: "",
    },
  ],
  lexiconEntries: [
...
function fixture() {
  ...
  const exportPrompt = vi.fn(
    async (
      ...
      fileName: kind === "creation" ? creation.fileName : update.fileName,
    }),
  );
  const exportSkillPackage = vi.fn(async () => ({
    disposition: "download" as const,
    fileName: "caching-skill.zip",
  }));
  const previewPrompt = vi.fn(
    async (_projectId: string | null, kind: "creation" | "update") =>
      kind === "creation" ? creation : update,
  );
  ...
  return render(
    <MemoryRouter initialEntries={[path]}>
      <Routes>
        ...
        />
        <Route
          path="/projects/:projectId/script-generation"
          element={
            <ScriptGenerationPage
              persistence={persistence}
              generation={generation}
...
describe("script prompt kit", () => {
  it("shows separate creation and update boilerplates without the saved script", async () => {
    ...
    expect(
      screen.queryByRole("link", { name: "View Projects" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /both prompts/u }),
    ).not.toBeInTheDocument();
  ...
  it("preserves independent drafts and copies and exports the selected edits", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Download update prompt" }),
    );
  ...
  it("disables copy and export for an empty draft", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Download creation prompt" }),
    ).toBeDisabled();
  });
  ...
  it("supports keyboard navigation between prompt tabs", async () => {
    ...
    expect(screen.getByRole("tab", { name: "Update Prompt" })).toHaveAttribute(
      "aria-selected",
      "true",
    );
  });
  ...
  it("keeps the existing project-specific route available", async () => {
    ...
    renderPage(
      persistence,
      generation,
      `/projects/${project.id}/script-generation`,
    );
    expect(
      await screen.findByRole("link", { name: `Back to ${project.name}` }),
    ).toBeInTheDocument();
