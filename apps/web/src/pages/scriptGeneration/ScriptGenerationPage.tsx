...
export function ScriptGenerationPage({
  ...
  const [documents, setDocuments] = useState<
    Partial<Record<ScriptPromptKind, PromptDocument>>
  >({});
  const [drafts, setDrafts] = useState<
    Partial<Record<ScriptPromptKind, string>>
  >({});
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .catch((error: unknown) => {
        if (active) setLoadingError(message(error));
      });
    return () => {
      active = false;
    };
      ...
      event.key === "Home"
        ...
        : event.key === "End"
          ...
          : event.key === "ArrowRight"
            ? (currentIndex + 1) % promptKinds.length
            : event.key === "ArrowLeft"
              ? (currentIndex - 1 + promptKinds.length) % promptKinds.length
              : undefined;
  ...
  const copyPrompt = async () => {
    ...
    try {
      ...
      setNotice(
        `${promptLabels[selected].title} prompt copied. Add your material in the marked block before sending it to an LLM.`,
      );
    } catch {
      setOperationError(
        "Clipboard access was denied. Download the prompt or copy it from the preview.",
      );
    } finally {
      setBusy(null);
    }
    ...
    try {
      ...
      setNotice(
        result.disposition === "canceled"
          ? "Prompt export canceled."
          : `${result.fileName} ${result.disposition === "saved" ? "saved" : "downloaded"}.`,
      );
    } catch (error) {
  ...
  if (
    !documents.creation ||
    !documents.update ||
    drafts.creation === undefined ||
    drafts.update === undefined ||
    !promptDocument ||
    (projectId && !project)
  )
    return (
      <section className={styles.loadingState} aria-live="polite">
        <h2>Preparing prompt kit</h2>
        <p>Reading the script format and pronunciation lexicon…</p>
      </section>
    );
      ...
      <header className={styles.header}>
        <div>
          ...
          <p>
            ...
            then use it with the LLM of your choice.
          </p>
        </div>
        {project ? (
        ...
        <StickyTabBar
          ...
          actions={
            <>
              <button
                type="button"
                aria-label={`Copy ${selected} prompt`}
              ...
              <button
                type="button"
                aria-label={`Download ${selected} prompt`}
                title={`Download ${selected} prompt`}
