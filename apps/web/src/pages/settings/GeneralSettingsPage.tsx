...
export function GeneralSettingsPage({
  ...
  const selectedSpeechModel = speechModels.find(
    ({ modelId }) => modelId === draft.defaultModelId,
  );
  ...
  const defaultVoiceOptions = useMemo(
    () => presentedVoices.filter(({ availableOnServer }) => availableOnServer),
    [presentedVoices],
  );
  ...
  const refreshSpeechCatalog = async () => {
    ...
    try {
      ...
      const voice =
        model.voices.find(({ voiceId }) => voiceId === draft.defaultVoiceId) ??
        model.voices[0];
      if (!voice)
        throw new Error(`Model ${model.modelId} did not report any voices.`);
      setDraft((current) => ({
        ...current,
      ...
      setStatus(
        `Loaded ${String(discovered.models.length)} speech ${discovered.models.length === 1 ? "model" : "models"}.`,
      );
    } catch (reason) {
      setError(
        reason instanceof Error
          ? reason.message
          : "The speech catalog could not be loaded.",
      );
    }
  };
  ...
  const saveConnection = async () => {
    ...
    try {
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : "The Speaches connection could not be saved and tested.",
      );
    }
  };
  ...
  const exportDiagnostics = async () => {
    try {
      ...
      anchor.href = url;
      anchor.download = "studynarrator-connection-diagnostics.json";
      anchor.click();
  ...
  const clearAllCache = async () => {
    ...
    try {
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : "The speech cache could not be cleared.",
      );
    } finally {
      setCacheBusy(false);
    }
      ...
      <section className={styles.connections}>
        <div className={styles.sectionHeading}>
          ...
          <button
            ...
            disabled={!draft.baseUrl || workspace.catalog.status === "loading"}
            onClick={() => void refreshSpeechCatalog()}
          >
        ...
        {workspace.connection ? (
            ...
            <div className={styles.connectionForm}>
                ...
                <select
                  ...
                  onChange={(event) => {
                    ...
                    setDraft({
                      ...
                      defaultVoiceId:
                        speechModels.find((model) => model.modelId === modelId)
                          ?.voices[0]?.voiceId ?? "",
                    });
                  }}
                >
                  <option value="">Load catalog to choose</option>
              ...
              <label>
                ...
                <VoiceSelect
                  ...
                  voices={defaultVoiceOptions}
                  disabled={!selectedSpeechModel}
                  emptyOption="Choose a voice"
                  onChange={(defaultVoiceId) =>
                    setDraft({ ...draft, defaultVoiceId })
                  }
                />
              </label>
              ...
              <div className={styles.actions}>
                ...
                >
                  {workspace.testing ? "Testing…" : "Save and Test"}
                </button>
              </div>
            </div>
            {showConnectionDiagnostics && connectionSummary ? (
              <div className={styles.diagnostics}>
                ...
                <ol>
                  {connectionSummary.stages.map((item, index) => (
                    <li data-status={item.status} key={item.stage}>
                      <b>{String(index + 1).padStart(2, "0")}</b>
                      <div>
                        <strong>{item.stage}</strong>
                        <code>
                          {item.code} · {item.durationMs} ms
                        </code>
          ...
          <div
            ...
            role="status"
            aria-label="Restoring connection settings"
          >
            <strong>Restoring saved connection settings…</strong>
            <span>
              The connection workshop will return when the application service
              is available.
            </span>
          </div>
        )}
          ...
          <>
            <div className={styles.cacheGrid}>
              <article>
                <span>Stored</span>
                <strong>
                  {cacheStatus.entryCount.toLocaleString()} entries
                </strong>
                <code>{formatBytes(cacheStatus.totalBytes)}</code>
              </article>
              <article>
                <span>This session</span>
                <strong>
                  {cacheStatus.sessionHits.toLocaleString()} hits ·{" "}
                  {cacheStatus.sessionMisses.toLocaleString()} misses
                </strong>
                <code>
                  {cacheStatus.sessionWrites.toLocaleString()} writes ·{" "}
                  {cacheStatus.sessionCorruptMisses.toLocaleString()} corrupt
                  misses
                </code>
              </article>
              <article>
                <span>Activity</span>
                <strong>
                  {cacheStatus.inFlight.toLocaleString()} in flight
                </strong>
                <code>
                  {cacheStatus.lastUsedAt
                    ? `Last used ${new Date(cacheStatus.lastUsedAt).toLocaleString()}`
                    : "Not used yet"}
                </code>
              </article>
            </div>
