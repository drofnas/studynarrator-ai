...
function nearestNamedTransition(
  ...
  const nearest = timing.pausePresets.reduce((closest, candidate) =>
    Math.abs(candidate.durationMs - setting.durationMs) <
    Math.abs(closest.durationMs - setting.durationMs)
      ? candidate
      : closest,
  );
...
function timingWithNamedTransitions(
  timing: SystemTimingConfiguration,
): SystemTimingConfiguration {
  return {
    ...timing,
    transitionPauses: {
      paragraph: nearestNamedTransition(
        timing.transitionPauses.paragraph,
        timing,
      ),
      speakerChange: nearestNamedTransition(
        timing.transitionPauses.speakerChange,
        timing,
      ),
...
function TimingTransitionEditor({
  ...
  return (
    <fieldset className={styles.transitionField}>
      <legend>{label}</legend>
      <label>
        Pause
        <select
          value={setting.mode === "preset" ? setting.pauseId : "none"}
          onChange={(event) => {
            const pauseId = event.target.value;
            onSettingChange(
              pauseId === "none"
                ? { mode: "none" }
                : { mode: "preset", pauseId: pauseId as SupportedPauseId },
            );
          }}
        >
          <option value="none">None</option>
          {SUPPORTED_PAUSE_IDS.map((pauseId) => (
            <option key={pauseId} value={pauseId}>
              {pauseId}
            </option>
          ))}
        </select>
...
export function TimingsSettingsPage({ client }: { client: PersistenceClient }) {
  ...
  const [pauseInputs, setPauseInputs] = useState<Record<string, string>>(() =>
    Object.fromEntries(
      DEFAULT_SYSTEM_TIMING.pausePresets.map((preset) => [
        preset.pauseId,
  ...
  const saveTiming = async () => {
    const parsedPresets = timing.pausePresets.map((preset) => ({
      preset,
      parsed: parsePauseDuration(pauseInputs[preset.pauseId] ?? ""),
    }));
    const invalidPreset = parsedPresets.find(({ parsed }) => !parsed.ok);
    if (invalidPreset && !invalidPreset.parsed.ok) {
      setError(
        `${invalidPreset.preset.pauseId}: ${invalidPreset.parsed.message}`,
      );
      return;
    }
    try {
      const saved = await client.settings.updatePacing({
        pausePresets: parsedPresets.map(({ preset, parsed }) => ({
          ...preset,
          durationMs: parsed.ok ? parsed.durationMs : preset.durationMs,
      ...
      <section className={styles.pacing} aria-labelledby="timing-heading">
        ...
        <div className={styles.pauseTableScroll}>
          <table className={styles.pauseTable}>
            ...
            <tbody>
              {timing.pausePresets.map((preset, index) => (
                <tr key={preset.pauseId}>
                  <th scope="row">
                    <code>{preset.pauseId}</code>
                  </th>
                  <td>
                    <label>
                      <span className={styles.srOnly}>
                        {preset.pauseId} duration
                      </span>
                      <input
        ...
        <div className={styles.transitionGrid}>
          {(["paragraph", "speakerChange", "section"] as const).map((key) => (
            <TimingTransitionEditor
              key={key}
              label={
                key === "speakerChange"
                  ? "Speaker change"
        ...
        <button type="button" onClick={() => void saveTiming()}>
          Save timing
        </button>
      </section>
    </div>
