...
export const scratchpadResult = {
  ...
  id: "b3b58e96-e98f-4dbf-897b-e2fb4b3a7c5c",
  createdAt: timestamp,
  modelId: "model-b",
  voiceId: "voice-b2",
  voiceLabel: "Second",
...
export const scratchpadClient = {
  preview: vi.fn(() => Promise.resolve(scratchpadResult)),
  ...
  return {
    ...
    lastTestSummary: {
      ...
      stages: stages.map((stage) => ({
        stage,
        status: overall === "connected" ? ("pass" as const) : ("fail" as const),
        code: `TEST_${stage.toUpperCase()}`,
      ...
      Promise.resolve({
        ...
        models: [
          ...
          },
          {
            modelId: "model-a",
            voices: [
              { voiceId: "voice-a1", name: null, language: null, gender: null },
            ],
          },
        ],
      }),
...
export const voiceCatalog: VoiceCatalogClient = {
  get: vi.fn((modelId: string) =>
    Promise.resolve({ schemaVersion: 1 as const, modelId, entries: [] }),
  ),
  replace: vi.fn(),
};
