...
import { connectionClient, scratchpadClient, scratchpadResult } from "./settingsTestFixtures.js";
...
describe("Voices settings", () => {
  ...
  it("searches locales and persists favorites with rollback on failure", async () => {
    ...
    render(<ConnectionProvider connectionClient={connectionClient()} voiceCatalog={localVoiceCatalog}><VoicesSettingsPage scratchpadClient={scratchpadClient} /></ConnectionProvider>);
  ...
  it("disables invalid auditions and aborts pending synthesis on unmount", async () => {
    ...
    const localVoiceCatalog = { get: vi.fn(async (modelId: string) => ({ schemaVersion: 1 as const, modelId, entries: [{ voiceId: "voice-b2", label: "Second voice", enabled: true, favorite: false, language: null, locale: null, accent: null, category: null, style: null, sampleText: null }] })), replace: vi.fn() };
    const view = render(<ConnectionProvider connectionClient={connectionClient()} voiceCatalog={localVoiceCatalog}><VoicesSettingsPage scratchpadClient={{ preview }} /></ConnectionProvider>);
