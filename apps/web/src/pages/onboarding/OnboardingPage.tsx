...
export function OnboardingPage() {
  ...
  const loadCatalog = async () => {
    ...
    try {
      ...
      if (!firstModel) throw new Error("This Speaches server did not report any speech models.");
      const firstVoice = firstModel.voices[0];
      if (!firstVoice) throw new Error(`Model ${firstModel.modelId} did not report any voices.`);
      setModelId(firstModel.modelId);
      setVoiceId(firstVoice.voiceId);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "The server catalog could not be loaded.");
    } finally { setBusy(false); }
  };

  const modelChanged = (nextModelId: string) => {
    setModelId(nextModelId);
    setVoiceId(catalog?.models.find(({ modelId: candidate }) => candidate === nextModelId)?.voices[0]?.voiceId ?? "");
  };
  ...
  const saveAndTest = async () => {
    ...
    setError("");
    try {
      await workspace.update({ baseUrl, defaultModelId: modelId, defaultVoiceId: voiceId });
      const result = await workspace.test();
      if (result.overall !== "connected") {
        setError(`Connection test finished with ${result.overall}. Review the staged result and try again.`);
        return;
      }
  ...
  return <div className={styles.page}>
    <header><p>First run</p><h2>Connect the voice workshop</h2><span>Enter your Speaches server, review its first available model and voice, then save and test.</span></header>
    <section className={styles.guide}>
      <div><span>01</span><h3>Enter the server address</h3><p>StudyNarrator supports unauthenticated HTTP(S) Speaches servers on loopback, your LAN, or HTTPS.</p></div>
    ...
    <form className={styles.form} onSubmit={(event) => { event.preventDefault(); void (catalog ? saveAndTest() : loadCatalog()); }}>
      <div className={styles.heading}><span>02</span><div><h3>Load and review the catalog</h3><p>The address is not saved until you choose Save and Test.</p></div></div>
      <label>Speaches address<input type="url" value={baseUrl} onChange={(event) => addressChanged(event.target.value)} placeholder="http://127.0.0.1:8000" required /></label>
      ...
      {catalog ? <>
        ...
        <label>Default Voice<select value={voiceId} onChange={(event) => setVoiceId(event.target.value)}>{selectedModel?.voices.map((voice) => <option key={voice.voiceId} value={voice.voiceId}>{voice.name ? `${voice.name} — ${voice.voiceId}` : voice.voiceId}</option>)}</select></label>
      </> : null}
