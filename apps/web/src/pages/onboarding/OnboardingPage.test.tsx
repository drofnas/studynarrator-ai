...
const connectedSummary: ConnectionTestSummary = {
  ...
  availableModelIds: ["model-z", "model-a"],
  availableVoiceIds: ["voice-z", "voice-a"],
};
const persistence = {
  projects: {
    list: vi.fn(async () => []),
    ...
    entries:
      modelId === "model-z"
        ? [
            {
              voiceId: "voice-z",
              label: "First Voice — English — voice-z",
              enabled: true,
              favorite: false,
              language: "English",
              locale: "en-US",
            ...
            {
              voiceId: "voice-y",
              label: "Favorite Voice — English — voice-y",
              enabled: true,
              favorite: true,
              language: "English",
              locale: "en-GB",
            ...
            {
              voiceId: "voice-x",
              label: "British Voice — English — voice-x",
              enabled: true,
              favorite: false,
              language: "English",
              locale: "en-GB",
  ...
  return render(
    <MemoryRouter initialEntries={[route]}>
      <App
        ...
        client={{ diagnostics: vi.fn() }}
        persistence={persistence}
        connection={connection}
        voiceCatalog={voiceCatalog}
        scratchpad={{ preview: vi.fn() }}
        projectPreview={{ preview: vi.fn() }}
...
describe("connection onboarding", () => {
  it("discovers a draft and preselects the first returned model and voice", async () => {
    ...
    await userEvent.type(
      screen.getByLabelText("Speaches address"),
      "http://127.0.0.1:8000",
    );
    ...
    expect(
      await screen.findByRole("option", {
        name: "First Voice (voice-z | en-US)",
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: "Favorite Voice (voice-y | en-GB)" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: "British Voice (voice-x | en-GB)" }),
    ).toBeInTheDocument();
    expect(
      [
        ...screen.getByLabelText("Default Voice").querySelectorAll("optgroup"),
      ].map(({ label }) => label),
    ).toEqual(["Favorites", "en-US", "en-GB"]);
    ...
    expect(
      await screen.findByRole("option", {
        name: "Other Voice (voice-a | fr-FR)",
      }),
    ).toBeInTheDocument();
