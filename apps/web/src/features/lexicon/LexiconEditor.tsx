...
function duplicateMessage(
  entries: readonly LexiconEditorValue[],
  displayText: string,
  label: string,
  exceptIndex?: number,
): string {
  ...
  return entries.some(
    (entry, index) =>
      index !== exceptIndex &&
      entry.displayText.trim().toLocaleLowerCase("en-US") === key,
  )
...
export function LexiconEditor({
    ...
    () =>
      value
        ...
        .filter(
          ({ entry }) =>
            !search ||
            `${entry.displayText} ${entry.spokenText}`
              .toLocaleLowerCase()
  ...
  const submit = async () => {
    ...
    const saved = await onChange(
      [...value, { id, displayText, spokenText, enabled: true }],
      { kind: "add", id },
    );
    setPending(false);
    if (saved !== false) {
  ...
  const update = (
    index: number,
    id: string,
    field: "displayText" | "spokenText",
    nextValue: string,
  ) => {
    if (field === "displayText")
      setValidationError(
        duplicateMessage(value, nextValue, displayTextLabel, index),
      );
    else if (!nextValue.trim())
      setValidationError(`${displayTextLabel} and Spoken Text are required.`);
    else setValidationError("");
    void onChange(
      value.map((entry, candidateIndex) =>
        candidateIndex === index ? { ...entry, [field]: nextValue } : entry,
      ),
      { kind: "edit", id },
    );
  };
  ...
  return (
    <div className={styles.editor}>
      ...
      {!hideSearchWhenEmpty || value.length > 0 ? (
        <input
          className={styles.search}
          aria-label={searchLabel}
          placeholder={`Search ${displayTextLabel} or Spoken Text`}
          value={search}
      ...
      <div className={styles.entries}>
        {filtered.length === 0 ? (
          ...
          filtered.map(({ entry, index, id }) => {
            return (
              <article
                key={id}
                aria-label={`Lexicon entry ${entry.displayText || `without ${displayTextLabel}`}`}
              >
                <label>
                  {displayTextLabel}
                  <input
                    ...
                    onBlur={() =>
                      void onChange([...value], { kind: "commit", id })
                    }
                  />
                </label>
                <span aria-hidden="true">→</span>
