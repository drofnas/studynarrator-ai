...
function duplicateMessage(entries: readonly LexiconEditorValue[], displayText: string, label: string, exceptIndex?: number): string {
  const key = displayText.trim().toLocaleLowerCase("en-US");
  if (!key) return `${label} and Spoken Text are required.`;
  return entries.some((entry, index) => index !== exceptIndex && entry.displayText.trim().toLocaleLowerCase("en-US") === key)
    ? `${label} must be unique regardless of capitalization.`
    : "";
}

export function LexiconEditor({ value, onChange, searchLabel, emptyMessage, disabled = false, hideSearchWhenEmpty = false, rowErrors = {}, displayTextLabel = "Script Text" }: LexiconEditorProps) {
  ...
  const submit = async () => {
    ...
    const validation = !displayText || !spokenText
      ? `${displayTextLabel} and Spoken Text are required.`
      : duplicateMessage(value, displayText, displayTextLabel);
  ...
  const update = (index: number, id: string, field: "displayText" | "spokenText", nextValue: string) => {
    if (field === "displayText") setValidationError(duplicateMessage(value, nextValue, displayTextLabel, index));
    else if (!nextValue.trim()) setValidationError(`${displayTextLabel} and Spoken Text are required.`);
  ...
  return <div className={styles.editor}>
    <form className={styles.add} onSubmit={(event) => { event.preventDefault(); void submit(); }}>
      <label>{displayTextLabel}<input disabled={disabled || pending} value={draft.displayText} onChange={(event) => { setDraft((current) => ({ ...current, displayText: event.target.value })); setValidationError(""); }} /></label>
    ...
    {!hideSearchWhenEmpty || value.length > 0 ? <input className={styles.search} aria-label={searchLabel} placeholder={`Search ${displayTextLabel} or Spoken Text`} value={search} onChange={(event) => setSearch(event.target.value)} /> : null}
    <div className={styles.entries}>{filtered.length === 0 ? <p>{emptyMessage}</p> : filtered.map(({ entry, index, id }) => {
      return <article key={id} aria-label={`Lexicon entry ${entry.displayText || `without ${displayTextLabel}`}`}>
        <label>{displayTextLabel}<input disabled={disabled || pending} value={entry.displayText} onChange={(event) => update(index, id, "displayText", event.target.value)} onBlur={() => void onChange([...value], { kind: "commit", id })} /></label>
        <span aria-hidden="true">→</span>
