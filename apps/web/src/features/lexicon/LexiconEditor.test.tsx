...
describe("LexiconEditor", () => {
  it("emits complete JSON values for add, inline edit, toggle, blur, and delete", async () => {
    ...
    expect(changes.at(-1)?.value.at(-1)).toMatchObject({
      displayText: "URL",
      spokenText: "U R L",
      enabled: true,
    });
    ...
    await user.type(
      within(apiRow).getByLabelText("Spoken Text"),
      "application programming interface",
    );
    await user.tab();
    expect(
      changes.some(
        ({ change }) => change.kind === "commit" && change.id === "api",
      ),
    ).toBe(true);
    await user.click(within(apiRow).getByLabelText("Enabled"));
    expect(changes.at(-1)).toMatchObject({
      change: { kind: "toggle", id: "api" },
    });
  ...
  it("searches both fields, rejects blanks and case-insensitive duplicates, and renders row errors", async () => {
    ...
    render(
      <LexiconEditor
        ...
        searchLabel="Search lexicon"
        emptyMessage="No entries."
        rowErrors={{ sql: "Not saved — edit or blur to retry" }}
      />,
    );
    const user = userEvent.setup();

    expect(
      screen.getByText("Not saved — edit or blur to retry"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByRole("article", { name: "Lexicon entry API" }),
    ).not.toBeInTheDocument();
    await user.clear(screen.getByLabelText("Search lexicon"));

    await user.click(screen.getByRole("button", { name: "Add" }));
    expect(
      screen.getByText("Script Text and Spoken Text are required."),
    ).toBeInTheDocument();
  ...
  it("uses Alias language without changing the emitted editor shape", async () => {
    ...
    expect(
      screen.getByPlaceholderText("Search Alias or Spoken Text"),
    ).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Add" }));
    expect(
      screen.getByText("Alias and Spoken Text are required."),
    ).toBeInTheDocument();
  ...
  it("supports JSON entries without IDs", async () => {
    ...
    await user.type(
      within(row).getByLabelText("Spoken Text"),
      "hypertext transfer protocol",
    );
    expect(within(row).getByLabelText("Spoken Text")).toHaveValue(
      "hypertext transfer protocol",
    );
  ...
  it("can hide search while empty and restore that state after the last deletion", async () => {
    ...
    const user = userEvent.setup();
    expect(
      screen.queryByLabelText("Search project lexicon"),
    ).not.toBeInTheDocument();
    await user.type(screen.getAllByLabelText("Script Text")[0]!, "API");
