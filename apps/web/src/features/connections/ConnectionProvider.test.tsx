...
function Consumer() {
  ...
  return (
    <div>
      ...
      <span>
        {workspace.loading ? "connection loading" : "connection loaded"}
      </span>
      ...
      <button type="button" onClick={() => void workspace.test()}>
        Test connection
      </button>
...
describe("ConnectionProvider", () => {
  it("loads the singleton and refreshes it after a connection test", async () => {
    ...
    expect(discoverSpeechCatalog).toHaveBeenCalledWith({
      baseUrl: connection.baseUrl,
      timeoutSeconds: 120,
      retryCount: 2,
    });
  ...
  it("recovers when the initial singleton load fails during an application restart", async () => {
    ...
    const connectionClient: SpeechBackendConnectionClient = {
      ...
      update: vi.fn(async () => connection),
      test: vi.fn(async () => summary),
      discoverSpeechCatalog: vi.fn(async () => catalog),
      exportDiagnostics: vi.fn(),
      getSetupState: vi.fn(async () => ({
        onboardingCompletedAt: timestamp,
        client: "web" as const,
