...
function parsePort(value: string | undefined): number {
  ...
  if (!/^\d+$/u.test(value))
    throw new Error(
      "STUDYNARRATOR_PORT must be an integer between 1 and 65535.",
    );
...
function parseHost(value: string | undefined): string {
  ...
  if (/\s/u.test(host))
    throw new Error(
      "STUDYNARRATOR_LISTEN_HOST must be a hostname or IP address without whitespace.",
    );
  return host;
}
...
): ServerRuntimeConfiguration["distribution"] {
  const distribution = value?.trim() || "development-web";
  if (
    !DISTRIBUTIONS.has(
      distribution as ServerRuntimeConfiguration["distribution"],
    )
  ) {
    throw new Error(
      "STUDYNARRATOR_DISTRIBUTION must be development-web or docker-web.",
    );
  }
...
function parseSourceRevision(value: string | undefined): string {
  ...
  if (sourceRevision.length > 128 || /[\r\n]/u.test(sourceRevision)) {
    throw new Error(
      "STUDYNARRATOR_SOURCE_REVISION must be a single value no longer than 128 characters.",
    );
  }
