...
describe("bundled Kokoro v1.0 catalog", () => {
  it("contains the complete upstream identifier set with neutral labels", () => {
    ...
    expect(KOKORO_V1_VOICE_CATALOG.entries.find(({ voiceId }) => voiceId === "af_heart")?.label)
      .toBe("Heart — American English — af_heart");
    expect(KOKORO_V1_VOICE_CATALOG.entries.some(({ label }) => /best|quality|grade/iu.test(label))).toBe(false);
  });
