...
import type { ConnectionProfilePlaceholder, RenderArtifact, RenderJob, RenderSegment } from "@studynarrator/shared-types";
import {
  createPcmSilence,
  probeAudioFile,
...
async function fixture() {
  ...
  const project = {
    contractVersion: 4 as const, id: projectId, name: "Render fixture", description: "", scriptSource: "narrator: Render me.",
    scriptHash: sha("narrator: Render me."), connectionProfileId: profileId, modelId: "model",
  ...
  const snapshot = withProjectSnapshotHash({
    schemaVersion: 1, capturedAt: timestamp, project, globalLexiconEntries: [], ignoredDiagnostics: [],
    connection: { profileId, profileName: "Local", profileSource: "saved", modelId: "model", serverIdentityHash: sha(baseUrl) },
  ...
  const repository = new MemoryRepository({
    id: profileId, name: "Local", baseUrl, suppliedUrlForm: "root", source: "saved", editable: true,
    ...
    speech: { async synthesize() {
      return {
        ...
        metadata: {
          ...
          serverIdentityHash: sha(baseUrl), profileId, modelId: "model", voiceId: "voice", speed: 1,
