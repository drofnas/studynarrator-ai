...
class ProjectPreviewServiceError extends Error {
  constructor(
    readonly code: ProjectPreviewServiceErrorCode,
    message: string,
  ) {
    super(message);
  }
...
export interface ProjectPreviewRepository
  extends
    ConnectionRepository,
    Pick<PersistenceRepository, "getProject" | "listGlobalLexicon"> {}
...
function safePreviewError(error: unknown): ProjectPreviewServiceError {
    ...
    switch (error.code) {
      case "aborted":
        return new ProjectPreviewServiceError(
          "PROJECT_PREVIEW_ABORTED",
          "Project preview was cancelled.",
        );
      case "audioTooLarge":
        return new ProjectPreviewServiceError(
          "PROJECT_PREVIEW_INVALID_AUDIO",
          "The preview WAV exceeded the 5 MiB limit.",
        );
      ...
      case "configurationError":
        return new ProjectPreviewServiceError(
          "PROJECT_PREVIEW_CONFIGURATION",
          "The project preview settings are incomplete.",
        );
      case "invalidAudio":
        return new ProjectPreviewServiceError(
          "PROJECT_PREVIEW_INVALID_AUDIO",
      ...
      case "selectionRejected":
        return new ProjectPreviewServiceError(
          "PROJECT_PREVIEW_SELECTION_REJECTED",
          "Speaches rejected the selected model or voice.",
        );
      case "unavailable":
        return new ProjectPreviewServiceError(
          "PROJECT_PREVIEW_UNAVAILABLE",
  ...
  if (record?.code === "PERSISTENCE_NOT_FOUND") {
    return new ProjectPreviewServiceError(
      "PROJECT_PREVIEW_CONFIGURATION",
      "The project or Speaches connection no longer exists.",
    );
  }
  return new ProjectPreviewServiceError(
    "PROJECT_PREVIEW_UNAVAILABLE",
...
function projectConnection(
  ...
  if (!modelId)
    throw new ProjectPreviewServiceError(
      "PROJECT_PREVIEW_CONFIGURATION",
      "Choose a speech model before previewing.",
    );
...
function segmentProjection(
  project: ProjectDetail,
  input: Extract<ProjectPreviewInput, { mode: "segment" }>,
  ...
  if (parsed.errors.length > 0)
    throw new ProjectPreviewServiceError(
      "PROJECT_PREVIEW_INVALID_SEGMENT",
      "Resolve script errors before previewing this segment.",
    );
  const node = parsed.nodes.find(
    (candidate) => candidate.ordinal === input.nodeOrdinal,
  );
  if (!node || node.type !== "speech")
    throw new ProjectPreviewServiceError(
      "PROJECT_PREVIEW_INVALID_SEGMENT",
      "The selected row is not a current speech segment.",
    );
  ...
  if (!segment || transformed.errors.length > 0)
    throw new ProjectPreviewServiceError(
      "PROJECT_PREVIEW_INVALID_SEGMENT",
      "Resolve pronunciation errors before previewing this segment.",
    );
  ...
  if (!speaker?.voiceId)
    throw new ProjectPreviewServiceError(
      "PROJECT_PREVIEW_CONFIGURATION",
      `Speaker ${node.speakerId} needs a voice before previewing.`,
    );
  return {
    mode: input.mode,
...
function pronunciationProjection(
  ...
  const hasControl =
    parsed.nodes.some((node) => node.type !== "speech") ||
    /(^|[^\\])\[speaker_[A-Za-z0-9][A-Za-z0-9_-]*\]/u.test(input.text);
  if (parsed.errors.length > 0 || hasControl) {
    throw new ProjectPreviewServiceError(
      "PROJECT_PREVIEW_INVALID_SEGMENT",
      "Pronunciation preview accepts one plain speech sample without script controls.",
    );
  }
  const transformed = transformScript({
    parsedScript: parsed,
  ...
  if (!transformed.synthesisReady || transformed.segments.length === 0) {
    throw new ProjectPreviewServiceError(
      "PROJECT_PREVIEW_INVALID_SEGMENT",
      "Resolve pronunciation errors before previewing this sample.",
    );
  }
  const configured = project.speakerMappings.find(
    (candidate) => candidate.speakerId === speakerId,
  );
  const voiceId =
    configured?.voiceId ?? (speakerId === "narrator" ? defaultVoiceId : null);
  if (!voiceId)
    throw new ProjectPreviewServiceError(
      "PROJECT_PREVIEW_CONFIGURATION",
      `Speaker ${speakerId} needs a voice before previewing.`,
    );
  return {
    mode: input.mode,
    nodeOrdinal: null,
  ...
  return {
    async preview(projectIdValue, inputValue, signal) {
      try {
        ...
        const { connection, modelId } = projectConnection(
          project,
        ...
        const voiceLabel =
          dependencies.repository
            .getVoiceCatalogOverrides(modelId)
            .entries.find((entry) => entry.voiceId === projection.voiceId)
            ?.label ??
          BUNDLED_VOICE_CATALOGS.get(modelId)?.entries.find(
            (entry) => entry.voiceId === projection.voiceId,
          )?.label ??
          projection.voiceId;
