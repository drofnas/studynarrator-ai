...
interface ConnectionDiagnosticRunner {
  (
    input: Parameters<typeof diagnoseSpeaches>[0],
...
export class ConnectionCatalogError extends Error {
  constructor(
    readonly code: ConnectionCatalogErrorCode,
    message: string,
  ) {
    super(message);
  }
  ...
  if (error instanceof SpeachesCatalogError) {
    switch (error.code) {
      ...
      case "configurationError":
        return new ConnectionCatalogError(
          "CONNECTION_CATALOG_CONFIGURATION",
          "Enter a valid HTTP(S) Speaches address.",
        );
      ...
      case "unavailable":
        return new ConnectionCatalogError(
          "CONNECTION_CATALOG_UNAVAILABLE",
          "The Speaches server is unavailable. Check its address and try again.",
        );
    }
  }
  return new ConnectionCatalogError(
...
function setupState(
  state: { onboardingCompletedAt: string | null },
...
export function classifyEndpoint(
  baseUrl: string | null,
): "loopback" | "private" | "public" | "unconfigured" {
  ...
  if (ipVersion === 6)
    return /^(fc|fd|fe8|fe9|fea|feb)/u.test(hostname) ? "private" : "public";
...
function requestCounts(summary: ConnectionTestSummary) {
  const status = (name: ConnectionTestSummary["stages"][number]["stage"]) =>
    summary.stages.find((candidate) => candidate.stage === name)?.status;
  return {
    health: status("http") === "skipped" ? 0 : 1,
    models: status("authentication") === "skipped" ? 0 : 1,
    voices:
      status("voice") === "skipped" &&
      summary.stages.find((candidate) => candidate.stage === "voice")?.code ===
        "not-run"
        ? 0
        : 1,
...
export function createConnectionService(dependencies: {
  ...
  return {
    ...
    async discoverSpeechCatalog(inputValue, signal) {
      try {
        ...
        if (catalog.models.length === 0)
          throw new ConnectionCatalogError(
            "CONNECTION_CATALOG_EMPTY",
    ...
    exportDiagnostics(): Promise<RedactedConnectionDiagnostics> {
      return Promise.resolve().then(() => {
        ...
        return RedactedConnectionDiagnosticsSchema.parse({
          ...
          runtimeVersions: {
            node: dependencies.context.nodeVersion,
            electron: dependencies.context.electronVersion,
          },
    ...
    getSetupState: () =>
      Promise.resolve(
        setupState(
          dependencies.repository.getConnectionSetup(),
    ...
    completeOnboarding() {
      return Promise.resolve(
        setupState(
          dependencies.repository.completeConnectionOnboarding(),
          dependencies.context,
        ),
      );
    },
...
export function createVoiceCatalogService(dependencies: {
  ...
  const merge = (modelId: string): VoiceCatalog => {
    const bundled =
      dependencies.bundledCatalogs.get(modelId) ??
    ...
    const entries = new Map(
      bundled.entries.map((entry) => [entry.voiceId, entry]),
    );
  ...
  return {
    get(modelId) {
      return Promise.resolve(merge(modelId));
    },
    replace(inputValue) {
