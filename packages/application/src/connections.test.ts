...
class MemoryRepository implements ConnectionRepository {
    ...
    this.connection = {
      ...
      lastSuccessfulTestAt:
        summary.overall === "connected"
          ? summary.testedAt
          : this.connection.lastSuccessfulTestAt,
  ...
  completeConnectionOnboarding() {
    this.setup.onboardingCompletedAt = timestamp;
    return this.getConnectionSetup();
  }
  getVoiceCatalogOverrides(modelId: string): VoiceCatalog {
    return (
      this.overrides.get(modelId) ?? {
...
function service(
  ...
  discoverCatalog: (
    input: Parameters<ConnectionCatalogRunner>[0],
  ) => Promise<SpeechCatalog> = vi.fn(async () => ({
    schemaVersion: 1 as const,
    models: [
      {
        modelId: "model",
...
describe("connection service", () => {
  it("executes every singular connection and voice-catalog service method", async () => {
    ...
    await expect(connection.get()).resolves.toMatchObject({
      baseUrl: "http://127.0.0.1:8000",
      configured: true,
    });
    await expect(
      connection.discoverSpeechCatalog({ baseUrl: "http://127.0.0.1:8000/v1" }),
    ).resolves.toMatchObject({ models: [{ modelId: "model" }] });
    await expect(connection.test()).resolves.toEqual(connected);
    await expect(connection.exportDiagnostics()).resolves.toMatchObject({
      endpointClass: "loopback",
    });
    await expect(connection.completeOnboarding()).resolves.toMatchObject({
    ...
    await expect(catalog.get("model")).resolves.toMatchObject({
      entries: [{ voiceId: "voice", enabled: false, favorite: true }],
    });
  });
  ...
  it("discovers a draft without persisting it and preserves response order", async () => {
    ...
    const discover = vi.fn(async () => ({
      ...
      models: [
        {
          modelId: "second-by-name",
          voices: [
            { voiceId: "z-voice", name: null, language: null, gender: null },
          ],
        },
        {
          modelId: "first-by-name",
          voices: [
            { voiceId: "a-voice", name: null, language: null, gender: null },
          ],
        },
      ],
    }));
    ...
    const result = await connection.discoverSpeechCatalog({
      baseUrl: "http://127.0.0.1:8000/v1",
      timeoutSeconds: 15,
      retryCount: 1,
    });
    expect(result.models.map(({ modelId }) => modelId)).toEqual([
      "second-by-name",
      "first-by-name",
    ]);
  ...
  it("reports empty catalogs and allows explicit offline onboarding", async () => {
    ...
    const connection = service(
      repository,
      vi.fn(async () => ({ schemaVersion: 1 as const, models: [] })),
    );
    await expect(
      connection.discoverSpeechCatalog({ baseUrl: "http://127.0.0.1:8000" }),
    ).rejects.toMatchObject({ code: "CONNECTION_CATALOG_EMPTY" });
    await expect(connection.completeOnboarding()).resolves.toMatchObject({
      onboardingCompletedAt: timestamp,
    });
...
describe("endpoint redaction", () => {
  it.each([
    ...
    "classifies %s without returning a hostname",
    (url, classification) => {
