...
describe("project preview service", () => {
  it("matches the project preview and speech cache application-service manifests", () => {
    const preview = createProjectPreviewService({
    ...
    expect(Object.keys(preview).map((key) => `projectPreview.${key}`)).toEqual(
      APPLICATION_SERVICE_MANIFEST.filter((path) =>
        path.startsWith("projectPreview."),
      ),
    );
    expect(Object.keys(cache).map((key) => `speechCache.${key}`)).toEqual(
      APPLICATION_SERVICE_MANIFEST.filter((path) =>
        path.startsWith("speechCache."),
      ),
    );
  });
  ...
  it("recomputes and synthesizes the selected speech segment with cache metadata", async () => {
    ...
    const result = await service.preview(projectId, {
      mode: "segment",
      nodeOrdinal: 1,
    });
  ...
  it("uses the connection default for a System narrator pronunciation sample", async () => {
    const speech = {
      synthesize: vi.fn(async () => ({
        ...cached(),
        metadata: { ...cached().metadata, voiceId: "voice-default", speed: 1 },
      })),
    };
    const service = createProjectPreviewService({
      repository: repository(),
  ...
  it("uses a configured narrator voice at one-times speed", async () => {
    ...
    const speech = {
      synthesize: vi.fn(async () => ({
        ...cached(),
        metadata: { ...cached().metadata, voiceId: "voice-narrator", speed: 1 },
      })),
    };
    const service = createProjectPreviewService({ repository: store, speech });
    await service.preview(projectId, {
      mode: "pronunciation",
      text: "System sample.",
      speakerId: "narrator",
    });
    expect(speech.synthesize).toHaveBeenCalledWith(
      expect.objectContaining({ voiceId: "voice-narrator", speed: 1 }),
    );
  });
  ...
  it("rejects pause rows and control-bearing pronunciation samples without synthesis", async () => {
    ...
    await expect(
      service.preview(projectId, { mode: "segment", nodeOrdinal: 3 }),
    ).rejects.toMatchObject({ code: "PROJECT_PREVIEW_INVALID_SEGMENT" });
    await expect(
      service.preview(projectId, {
        mode: "pronunciation",
        text: "[pause_short]",
      }),
    ).rejects.toMatchObject({ code: "PROJECT_PREVIEW_INVALID_SEGMENT" });
