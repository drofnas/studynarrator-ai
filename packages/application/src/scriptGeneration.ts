...
type ScriptGenerationServiceErrorCode =
  "SCRIPT_GENERATION_NOT_FOUND" | "SCRIPT_GENERATION_STORAGE";

class ScriptGenerationServiceError extends Error {
  constructor(
    readonly code: ScriptGenerationServiceErrorCode,
    message: string,
  ) {
    super(message);
  }
...
export type ScriptGenerationRepository = Pick<
  PersistenceRepository,
  "getProject" | "getSystemPacing" | "listGlobalLexicon"
>;
    ...
    speakers:
      project && project.speakerMappings.length > 0
        ? project.speakerMappings.map(
            ({ speakerId, displayName, roleDescription }) => ({
              speakerId,
              roleDescription:
                roleDescription.trim() ||
                `${displayName} contributes clear spoken explanations.`,
            }),
          )
        : [
            {
              speakerId: "narrator",
              roleDescription: "Explains the material clearly and accurately.",
            },
          ],
    pauses: timing.pausePresets.map(({ pauseId, durationMs, description }) => ({
      pauseId,
      description:
        description.trim() || `${String(durationMs)} millisecond pause.`,
    })),
  });
}
...
function promptDocument(
  ...
  const content = buildExternalLlmPrompt({
    kind,
    context: generationContext(timing, project),
...
export function createScriptGenerationService(dependencies: {
  repository: ScriptGenerationRepository;
  ...
  const load = (
    ...
    const project = dependencies.repository.getProject(
      ProjectIdSchema.parse(projectIdInput),
    );
    return {
      project,
      timing,
      entries: lexicon(dependencies.repository, project),
    };
  };
  return {
    async previewPrompt(projectIdInput, kindInput) {
      try {
        ...
        return await Promise.resolve(
          promptDocument(timing, project, kind, entries),
        );
      } catch (error) {
    ...
    async resolveSkillPackage(projectIdInput) {
      try {
        ...
        const archiveEntries: Record<string, [Uint8Array, { mtime: Date }]> =
          {};
        for (const file of buildSkillPackageFiles({
          context: generationContext(timing, project),
          lexiconEntries: entries,
        })) {
          archiveEntries[file.path] = [
