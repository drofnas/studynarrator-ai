...
describe("paragraph pacing preview", () => {
    ...
    expect(result.audits).toEqual([
      {
        ...
        paragraphBreaks: [
          { nodeOrdinal: 2, range: parsedScript.nodes[1]?.range },
        ],
  ...
  it("accepts zero and maximum durations while rejecting invalid configuration", () => {
    expect(
      ParagraphPauseConfigurationSchema.parse({
  ...
  it("collapses multiple contributing paragraph boundaries into one speech transition", () => {
    ...
    expect(
      result.audits[0]?.paragraphBreaks.map(({ nodeOrdinal }) => nodeOrdinal),
    ).toEqual([2, 4]);
  });
  ...
  it("audits explicit-pause suppression without removing authored pauses", () => {
    ...
    expect(
      parsedScript.nodes
        .filter((node) => node.type === "pause")
        .map(({ pauseId }) => pauseId),
    ).toEqual(["pause_short", "pause_long"]);
  });
  ...
  it("is deterministic and accepts deeply frozen parse results without mutation", () => {
    ...
    const first = resolveParagraphPauses({
      parsedScript,
      configuration: defaultConfiguration,
    });
    const second = resolveParagraphPauses({
      parsedScript,
      configuration: defaultConfiguration,
    });
