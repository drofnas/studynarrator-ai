...
describe("script parser", () => {
  it("parses a representative study guide with exact discoveries", () => {
    ...
    expect(result.discoveries.speakers.map(({ id }) => id)).toEqual([
      "teacher",
      "student",
    ]);
    expect(result.discoveries.pauses.map(({ id }) => id)).toEqual([
      "pause_short",
      "pause_long",
    ]);
  ...
  it("returns stable errors while preserving recoverable invalid lines as speech", () => {
    ...
    expect(
      result.errors.every(
        ({ offendingText, suggestion }) =>
          offendingText.length > 0 && suggestion.length > 0,
      ),
    ).toBe(true);
  });
  ...
  it("supports standalone, inline, multiple, and consecutive speaker directives", () => {
    ...
    expect(
      speech.map(({ speakerId, readableText }) => [speakerId, readableText]),
    ).toEqual([
      ["teacher", "Hello."],
      ["student", "Hi."],
    ...
    expect(
      result.discoveries.speakers.map(({ id, occurrences }) => [
        id,
        occurrences.length,
      ]),
    ).toEqual([
  ...
  it("requires the speaker_ namespace and never mistakes unknown directives for speakers", () => {
    const result = parseScript({
      source:
        "[speaker_sectioned] This is a speaker.\n[section Database indexes]",
    ...
    expect(result.nodes[1]).toMatchObject({
      type: "speech",
      speakerId: "sectioned",
      readableText: "[section Database indexes]",
    });
  });
  ...
  it("warns when speaker IDs differ only by case", () => {
    ...
    const ignored = parseScript({
      ...
      ignoredDiagnostics: [
        {
          code: "SPEAKER_ID_CASE_COLLISION",
          pattern: "[speaker_teacher] Second.",
        },
      ],
    });
  ...
  it("collapses blank-line runs into paragraph boundaries", () => {
    const result = parseScript({
      source: "[speaker_teacher] One.\n\n  \nTwo.",
    });
    const paragraph = result.nodes.find(
  ...
  it("treats escaped beginning brackets and middle brackets as readable speech", () => {
    const result = parseScript({
      source:
        "\\[not_a_speaker] spoken literally\nA sentence with [brackets] in its middle.",
      defaultSpeakerId: "narrator",
    });
    const speech = result.nodes.filter((node) => node.type === "speech");
    expect(result.errors).toEqual([]);
    expect(
      speech.map(({ rawText, readableText }) => [rawText, readableText]),
    ).toEqual([
      [
        "\\[not_a_speaker] spoken literally",
        "[not_a_speaker] spoken literally",
  ...
  it("escapes inline bracket tokens instead of treating them as directives", () => {
    const result = parseScript({
      source: "[speaker_teacher] Keep \\[pause_short] literal.",
    });
    expect(result.nodes[0]).toMatchObject({
      readableText: "Keep [pause_short] literal.",
    });
  ...
  it("extracts annotations and preserves escaped literal annotation markup", () => {
    const result = parseScript({
      source: "[speaker_teacher] Read {{resume|cv}} and \\{{example|literal}}.",
    });
    const speech = result.nodes.find((node) => node.type === "speech");
    expect(speech).toMatchObject({
      readableText: "Read resume and {{example|literal}}.",
      annotations: [
        { displayText: "resume", senseId: "cv", rawText: "{{resume|cv}}" },
      ],
    });
  ...
  it.each([
    ...
    [
      "[speaker_teacher] {{resume|cv|extra}}",
      "INVALID_PRONUNCIATION_ANNOTATION",
    ],
    ["[speaker_teacher] resume}}", "UNMATCHED_PRONUNCIATION_CLOSE"],
  ])(
    "diagnoses malformed pronunciation markup but retains it as literal speech in %s",
    (source, code) => {
      const result = parseScript({ source });
      expect(result.errors[0]?.code).toBe(code);
      expect(result.nodes[0]).toMatchObject({
        type: "speech",
        readableText: source.slice(source.indexOf("]") + 2),
        annotations: [],
      });
    },
  );
  ...
  it("uses the system narrator by default and honors explicit overrides", () => {
    ...
    const systemDefault = parseScript({
      source: "Opening text.\n[speaker_teacher] Assigned.",
    });
    expect(systemDefault.errors).toEqual([]);
    expect(
      systemDefault.nodes
        .filter((node) => node.type === "speech")
        .map(({ speakerId }) => speakerId),
    ).toEqual(["narrator", "teacher"]);
    expect(systemDefault.discoveries.speakers.map(({ id }) => id)).toEqual([
      "narrator",
      "teacher",
    ]);

    const override = parseScript({
      source: "Opening text.\n[speaker_teacher] Assigned.",
      defaultSpeakerId: "host",
    });
    ...
    expect(
      override.nodes
        .filter((node) => node.type === "speech")
        .map(({ speakerId }) => speakerId),
    ).toEqual(["host", "teacher"]);
    expect(override.discoveries.speakers.map(({ id }) => id)).toEqual([
      "host",
      "teacher",
    ]);
  });
  ...
  it("parses bare scripts with structure, annotations, and pauses under the system narrator", () => {
    ...
    expect(
      result.nodes.map((node) =>
        node.type === "speech"
          ? `${node.speakerId}: ${node.readableText}`
          : node.type,
      ),
    ).toEqual([
  ...
  it("parses hundreds of bare lines without missing-speaker diagnostics or source loss", () => {
    const source = Array.from(
      { length: 783 },
      (_, index) => `Study guide line ${String(index + 1)}.`,
    ).join("\n");
    ...
    expect(first.summary).toMatchObject({
      speakerCount: 1,
      speechSegmentCount: 783,
    });
    expect(
      first.nodes
        .filter((node) => node.type === "speech")
        .every(({ speakerId }) => speakerId === "narrator"),
    ).toBe(true);
  });

  it("preserves Unicode speech", () => {
    const result = parseScript({
      source: "[speaker_teacher] Καλημέρα 世界 — résumé.",
    });
    expect(result.nodes[0]).toMatchObject({
      readableText: "Καλημέρα 世界 — résumé.",
    });
  });
  ...
  it.each([
    ...
    "diagnoses malformed directive %s and keeps it as literal speech",
    (source, code) => {
  ...
  it("accepts numeric-leading speaker names and emits inline pause speech after the pause", () => {
    ...
    ).toEqual(["1bad: Before.", "pause", "1bad: After."]);
  });
  ...
  it("recognizes pause and speaker directives anywhere in speech and carries the speaker forward", () => {
    ...
    const result = parseScript({
      ...
      ignoredDiagnostics: [
        {
          code: "MALFORMED_SECTION_DIRECTIVE",
          pattern: "[section Database indexes]",
        },
    ...
    expect(
      result.nodes
        .filter((node) => node.type === "speech")
        .map(({ speakerId, readableText }) => [speakerId, readableText]),
    ).toEqual([
      [
        "1bad",
        "This speaker name begins with a number and can be mapped later.",
      ],
  ...
  it("diagnoses malformed speakers while treating unsupported pause-shaped text as speech", () => {
    const result = parseScript({
      source:
        "[speaker_teacher] Keep [speaker_bad name] and [pause_bad!] literal.",
    });
    expect(
      result.errors.map(({ code, ignorePattern }) => [code, ignorePattern]),
    ).toEqual([["INVALID_SPEAKER_DIRECTIVE", "[speaker_bad name]"]]);
    expect(result.nodes[0]).toMatchObject({
      type: "speech",
      speakerId: "teacher",
  ...
  it("keeps unsupported standalone and inline pause directives as ordinary speech", () => {
    ...
    expect(
      result.nodes
        .filter((node) => node.type === "speech")
        .map((node) => (node.type === "speech" ? node.readableText : "")),
    ).toEqual(["[pause_custom]", "Keep [pause_archived] literal."]);
    expect(
      transformScript({ parsedScript: result, entries: [] }).segments.map(
        ({ ttsText }) => ttsText,
      ),
    ).toEqual(["[pause_custom]", "Keep [pause_archived] literal."]);
  });
  ...
  it("suppresses every malformed annotation occurrence with the same token pattern", () => {
    ...
    expect(first.errors.map(({ offendingText }) => offendingText)).not.toEqual([
      first.errors[0]?.offendingText,
      first.errors[0]?.offendingText,
    ]);
    expect(first.errors.map(({ ignorePattern }) => ignorePattern)).toEqual([
      "{{resume|cv",
      "{{resume|cv",
    ]);
    expect(ignored.errors).toEqual([]);
  ...
  it("keeps malformed annotation text literal while still parsing valid annotations on the same line", () => {
    const result = parseScript({
      source:
        "[speaker_teacher] {{resume|cv}} then {{broken}} then {{job|continue}}.",
    });
    expect(result.errors.map(({ code }) => code)).toEqual([
      "INVALID_PRONUNCIATION_ANNOTATION",
    ]);
  ...
  it("is deterministic and line-ending equivalent while preserving exact source", () => {
    ...
    expect({
      ...
      summary: {
        ...windows.summary,
        characterCount: first.summary.characterCount,
      },
    }).toEqual(first);
    expect(JSON.stringify(first)).not.toMatch(
      /timestamp|createdAt|random|uuid/iu,
    );
  });

  it("validates public parser input instead of returning syntax diagnostics", () => {
    expect(() =>
      ParseScriptInputSchema.parse({
        source: "text",
        defaultSpeakerId: "bad speaker",
      }),
    ).toThrow();
    expect(() =>
      parseScript({
        source: "text",
        ignoredDiagnostics: [{ code: "bad", pattern: "text" }],
      }),
    ).toThrow();
  });
});
