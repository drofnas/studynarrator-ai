...
export function parsePauseDuration(value: string): PauseDurationParseResult {
  ...
  if (!source) return { ok: false, code: "EMPTY", message: "Enter a pause duration." };
  if (source.startsWith("-")) return { ok: false, code: "NEGATIVE", message: "Pause duration cannot be negative." };
  if (/^\d+(?:\.\d{4,})\s*s$/iu.test(source)) {
    return { ok: false, code: "SUB_MILLISECOND_PRECISION", message: "Seconds may use at most three decimal places." };
  }
  const match = /^(\d+)(?:\.(\d{1,3}))?\s*(ms|s)?$/iu.exec(source);
  if (!match) {
    return { ok: false, code: "INVALID_FORMAT", message: "Use whole milliseconds or seconds, such as 350 ms or 1.5 s." };
  }
  const whole = Number(match[1]);
  const fraction = match[2];
  const unit = match[3]?.toLowerCase() ?? "ms";
  if (unit === "ms" && fraction !== undefined) {
    return { ok: false, code: "INVALID_FORMAT", message: "Millisecond values must be whole numbers." };
  }
  const durationMs = unit === "s"
    ? whole * 1_000 + Number((fraction ?? "").padEnd(3, "0") || "0")
    : whole;
  if (!Number.isSafeInteger(durationMs) || durationMs > 30_000) {
    return { ok: false, code: "OUT_OF_RANGE", message: "Pause duration must be between 0 and 30,000 ms." };
  }
  return { ok: true, durationMs, normalized: `${String(durationMs)} ms` };
}

const starterPauses = new Map<string, { durationMs: number; description: string }>([
  ["pause_short", { durationMs: 350, description: "Brief thinking beat or speaker handoff." }],
  ["pause_medium", { durationMs: 750, description: "Paragraph or subtopic separation." }],
  ["pause_long", { durationMs: 1_500, description: "Major subject or section separation." }]
]);

function displayNameForSpeaker(speakerId: string): string {
  return speakerId.split(/[_-]+/u).filter(Boolean).map((part) => `${part[0]?.toUpperCase() ?? ""}${part.slice(1)}`).join(" ");
}
...
export function reconcileDiscoveredConfiguration(input: {
  ...
  const existingSpeakers = new Map(input.speakerMappings.map((item) => {
    const parsed = AuthoringSpeakerConfigurationSchema.parse(item);
    return [parsed.speakerId, parsed] as const;
  }));
  const existingPauses = new Map(input.pausePresets.map((item) => {
    const parsed = AuthoringPauseConfigurationSchema.parse(item);
    return [parsed.pauseId, parsed] as const;
  }));

  const speakers: AuthoringSpeakerRow[] = parseResult.discoveries.speakers.map((discovery) => ({
  ...
  const discoveredSpeakers = new Set(speakers.map(({ speakerId }) => speakerId));
  for (const speaker of existingSpeakers.values()) {
    if (!discoveredSpeakers.has(speaker.speakerId)) speakers.push({ ...speaker, discovered: false, occurrenceCount: 0 });
  }

  const pauses: AuthoringPauseRow[] = parseResult.discoveries.pauses.map((discovery) => {
  ...
  for (const pause of existingPauses.values()) {
    if (!discoveredPauses.has(pause.pauseId)) pauses.push({ ...pause, discovered: false, occurrenceCount: 0 });
  }
  ...
  for (const node of parseResult.nodes) {
    if (node.type === "section") {
      currentSection = { title: node.title, sourceLine: node.range.start.line, speechSegmentCount: 0 };
...
export function validateAuthoringConfiguration(input: {
  ...
  const parseResult = ParseScriptResultSchema.parse(input.parseResult);
  const transformResult = TransformScriptResultSchema.parse(input.transformResult);
  const speakers = input.speakers.map((item) => AuthoringSpeakerRowSchema.parse(item));
  const pauses = input.pauses.map((item) => AuthoringPauseRowSchema.parse(item));
  const issues: AuthoringValidationIssue[] = [];
  if (parseResult.summary.speechSegmentCount === 0) {
    issues.push({ code: "EMPTY_SCRIPT", severity: "error", message: "Add at least one speech segment before rendering.", target: { kind: "script", id: "source" } });
  }
  for (const item of parseResult.errors) issues.push({ code: item.code, severity: "error", message: item.message, line: item.line, column: item.column, target: { kind: "script", id: item.ignorePattern } });
  for (const item of parseResult.warnings) issues.push({ code: item.code, severity: "warning", message: item.message, line: item.line, column: item.column, target: { kind: "script", id: item.ignorePattern } });
  for (const item of transformResult.errors) issues.push({ code: item.code, severity: "error", message: item.message, line: item.range.start.line, column: item.range.start.column, target: { kind: "lexicon", id: item.ignorePattern } });
  for (const item of transformResult.warnings) issues.push({ code: item.code, severity: "warning", message: item.message, line: item.range.start.line, column: item.range.start.column, target: { kind: "lexicon", id: item.ignorePattern } });
  for (const speaker of speakers.filter(({ discovered }) => discovered)) {
    if (!speaker.voiceId?.trim()) issues.push({
  ...
  for (const pause of pauses.filter(({ discovered }) => discovered)) {
    if (pause.durationMs === null) issues.push({
  ...
  const status = hasErrors ? "blocked" : issues.length > 0 ? "readyWithWarnings" : "ready";
  return AuthoringValidationResultSchema.parse({ schemaVersion: AUTHORING_SCHEMA_VERSION, status, issues });
}
...
export const AuthoringDryRunResultSchema = z.object({
  schemaVersion: z.literal(AUTHORING_SCHEMA_VERSION),
  status: z.enum(["ready", "readyWithWarnings", "blocked"]),
  issues: z.array(AuthoringValidationIssueSchema),
...
export function buildAuthoringDryRun(input: {
  ...
  const parseResult = ParseScriptResultSchema.parse(input.parseResult);
  const pacingResult = ResolveParagraphPausesResultSchema.parse(input.pacingResult);
  const transformResult = TransformScriptResultSchema.parse(input.transformResult);
  const speakers = input.speakers.map((item) => AuthoringSpeakerRowSchema.parse(item));
  const pauses = input.pauses.map((item) => AuthoringPauseRowSchema.parse(item));
  const validation = validateAuthoringConfiguration({ parseResult, transformResult, speakers, pauses });
  const speakerById = new Map(speakers.map((item) => [item.speakerId, item]));
  const pauseById = new Map(pauses.map((item) => [item.pauseId, item]));
  const transformedByOrdinal = new Map(transformResult.segments.map((item) => [item.nodeOrdinal, item]));
  const automaticByParagraphOrdinal = new Map<number, ResolveParagraphPausesResult["audits"][number]>();
  for (const audit of pacingResult.audits) {
    const firstBreak = audit.paragraphBreaks[0];
    if (audit.status === "applied" && firstBreak) automaticByParagraphOrdinal.set(firstBreak.nodeOrdinal, audit);
  }

  const rows: DryRunRow[] = [];
  const push = (row: DryRunRowInput) => rows.push({ ...row, rowNumber: rows.length + 1 });
  for (const node of parseResult.nodes) {
    if (node.type === "section") {
      push({ type: "section", nodeOrdinal: node.ordinal, title: node.title, sourceRange: node.range, validationStatus: "valid" });
      ...
      push({
        ...
        sourceRange: node.range,
        validationStatus: configured?.durationMs === null || configured === undefined ? "error" : "valid"
      });
    } else {
  ...
  return AuthoringDryRunResultSchema.parse({ schemaVersion: AUTHORING_SCHEMA_VERSION, status: validation.status, issues: validation.issues, rows });
}
