...
const ScratchpadPassageInputSchema = z
  .object({
    text: z
      .string()
      .max(SCRATCHPAD_PASSAGE_MAX_CHARACTERS)
      .refine(
        (value) => value.trim().length > 0,
        "Enter a passage to synthesize.",
      ),
...
export function transformScratchpadPassage(inputValue: {
  ...
  if (firstError) {
    throw new ScratchpadPassageError(
      `${firstError.message} Use Projects for structured narration scripts.`,
    );
  }
  const hasControlNode = parsed.nodes.some(
    (node) => node.type === "section" || node.type === "pause",
  );
  const hasSpeakerControl =
    /(^|[^\\])\[speaker_[A-Za-z0-9][A-Za-z0-9_-]*\]/u.test(input.text);
  if (hasControlNode || hasSpeakerControl) {
    throw new ScratchpadPassageError(
      "Quick Scratchpad accepts speech text only. Use Projects for speaker, pause, and section controls.",
    );
  }
  ...
  if (
    ...
    transformed.ttsTranscript.trim().length === 0
  ) {
    throw new ScratchpadPassageError(
      "The passage could not be prepared for synthesis. Review the source-linked message and try again.",
    );
  }
  return {
