...
function lexiconEntry(
  overrides: Partial<LexiconEntry> &
    Pick<LexiconEntry, "id" | "displayText" | "spokenText">,
): LexiconEntry {
  return {
    scope: "global",
    entryType: "exactTerm",
...
describe("pause duration normalization", () => {
  it.each([
    ...
    expect(parsePauseDuration(input)).toEqual({
      ok: true,
      durationMs: expected,
      normalized: `${String(expected)} ms`,
    });
  });

  it("rejects negative, malformed, over-precise, and out-of-range values", () => {
    expect(parsePauseDuration("-1 s")).toMatchObject({
      ok: false,
      code: "NEGATIVE",
    });
    expect(parsePauseDuration("1.5 ms")).toMatchObject({
      ok: false,
      code: "INVALID_FORMAT",
    });
    expect(parsePauseDuration("0.0001 s")).toMatchObject({
      ok: false,
      code: "SUB_MILLISECOND_PRECISION",
    });
    expect(parsePauseDuration("30.001 s")).toMatchObject({
      ok: false,
      code: "OUT_OF_RANGE",
    });
    expect(parsePauseDuration("later")).toMatchObject({
      ok: false,
      code: "INVALID_FORMAT",
    });
  });
...
describe("discovery reconciliation", () => {
  it("adds supported defaults and preserves unused authored configuration", () => {
    const parseResult = parseScript({
      source: "[speaker_teacher] One. [pause_short] Two. [pause_custom] Three.",
    });
    ...
    expect(first.pauses).toEqual([
      ...
      expect.objectContaining({
        pauseId: "pause_archived",
        durationMs: 2_000,
        discovered: false,
      }),
    ]);

    const savedSpeakers = first.speakers.map(
      ({
    ...
    const savedPauses = first.pauses.flatMap(
      ({
        durationMs,
        discovered: _discovered,
        occurrenceCount: _occurrenceCount,
    ...
    const reloaded = reconcileDiscoveredConfiguration({
      parseResult,
      speakerMappings: savedSpeakers,
      pausePresets: savedPauses,
    });
  ...
  it("reports section lines and speech counts", () => {
    ...
    const result = reconcileDiscoveredConfiguration({
      parseResult,
      speakerMappings: [],
      pausePresets: [],
    });
...
describe("deterministic readiness and dry run", () => {
  it("blocks missing speakers while treating unsupported pause-shaped text as speech", () => {
    const parseResult = parseScript({
      source: "[speaker_teacher] Hello. [pause_custom] Continue.",
    });
    ...
    expect(validation.issues).toEqual([
      expect.objectContaining({
        ...
        target: { kind: "speaker", id: "teacher" },
      }),
    ]);
  });
  ...
  it("keeps original, readable, and TTS text separate in a representative study guide", () => {
    ...
    const pacingResult = resolveParagraphPauses({
      parsedScript: parseResult,
      configuration: {
        enabled: true,
    ...
    const reconciled = reconcileDiscoveredConfiguration({
      parseResult,
      speakerMappings: [
        speaker("teacher", "voice_teacher"),
        speaker("student", "voice_student"),
      ],
      pausePresets: [
        pause("pause_short", 350),
        pause("pause_medium", 750),
        pause("pause_long", 1_500),
      ],
    });
    const result = buildAuthoringDryRun({
      parseResult,
      pacingResult,
      transformResult,
      speakers: reconciled.speakers,
    ...
    if (firstSpeech?.type !== "speech")
      throw new Error("Expected a speech row.");
  ...
  it("inserts one automatic paragraph pause and never doubles an explicit pause", () => {
    ...
    const automaticTransform = transformScript({
      parsedScript: automaticParse,
      entries: [],
    });
    const automaticPacing = resolveParagraphPauses({
      parsedScript: automaticParse,
      configuration: {
        enabled: true,
        pauseId: "pause_medium",
    ...
    const automatic = buildAuthoringDryRun({
      parseResult: automaticParse,
      pacingResult: automaticPacing,
      transformResult: automaticTransform,
      speakers: automaticConfig.speakers,
      pauses: automaticConfig.pauses,
    });
    expect(
      automatic.rows.map((row) =>
        row.type === "pause" ? `${row.type}:${row.origin}` : row.type,
      ),
    ).toEqual(["speech", "pause:paragraph", "speech"]);

    const explicitParse = parseScript({
      source: "First.\n[pause_short]\n\nSecond.",
    });
    const explicitTransform = transformScript({
      parsedScript: explicitParse,
      entries: [],
    });
    const explicitPacing = resolveParagraphPauses({
      parsedScript: explicitParse,
      configuration: {
        enabled: true,
        pauseId: "pause_medium",
    ...
    const explicit = buildAuthoringDryRun({
      parseResult: explicitParse,
      pacingResult: explicitPacing,
      transformResult: explicitTransform,
      speakers: explicitConfig.speakers,
      pauses: explicitConfig.pauses,
    });
