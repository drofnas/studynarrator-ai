...
function transform(
  ...
  return transformScript({
    parsedScript: parseScript({ source, defaultSpeakerId }),
    entries,
  });
}
...
describe("lexicon transformation", () => {
  it("transforms a representative study guide while preserving its readable transcript and source", () => {
    ...
    expect(
      ...
      {
        entryId: "project-resume-cv",
        originalText: "resume",
        replacement: "rez-oo-may",
        nodeOrdinal: 3,
        sourceStartOffset: 113,
        sourceEndOffset: 119,
        line: 3,
        column: 68,
      },
      {
        entryId: "project-resume-continue",
        originalText: "resume",
        replacement: "ree-zoom",
        nodeOrdinal: 7,
        sourceStartOffset: 270,
        sourceEndOffset: 276,
        line: 7,
        column: 47,
      },
  ...
  it("uses the required scope and entry-type precedence", () => {
    ...
    const phraseBeforeTerm = transform("data base", [
      ...
      entry({
        id: "global-phrase",
        entryType: "exactPhrase",
        displayText: "data base",
        spokenText: "phrase",
      }),
    ]);
    ...
    expect(unresolved.warnings).toMatchObject([
      {
        ...
        sourceEndOffset:
          source.indexOf("{{resume|cv}}") + "{{resume|cv}}".length,
      },
    ]);
    ...
    const resolved = transformScript({
      ...
      entries: [
        ...
        entry({
          ...
          displayText: "resume",
          senseId: "cv",
          spokenText: "project-cv",
        }),
      ],
    });
    expect(resolved.ttsTranscript).toBe("project-cv and ordinary.");
    expect(resolved.matches.map(({ entryId }) => entryId)).toEqual([
      "project-cv",
      "ordinary-resume",
    ]);
  });
  ...
  it("uses the literal fallback for every ineligible named-sense entry", () => {
    const source =
      "[speaker_teacher] {{resume|cv}} {{Resume|cv}} {{resume|missing}}";
    const result = transformScript({
      ...
      entries: [
        ...
        entry({
          ...
          displayText: "RESUME",
          senseId: "cv",
          spokenText: "wrong case",
        }),
      ],
    });
    expect(result.readableTranscript).toBe(
      "{{resume|cv}} {{Resume|cv}} {{resume|missing}}",
    );
  ...
  it("produces deterministic non-overlapping audits with exact UTF-16 source offsets", () => {
    ...
    expect(
      first.matches.map(({ sourceStartOffset, sourceEndOffset }) => [
        sourceStartOffset,
        sourceEndOffset,
      ]),
    ).toEqual([
    ...
    expect(first.matches.map(({ range }) => range.start.column)).toEqual([
      19, 33,
    ]);
  });
  ...
  it("does not mutate a deeply frozen parse result or lexicon entries", () => {
    ...
    deepFreeze(entries);
    expect(transformScript({ parsedScript, entries }).ttsTranscript).toBe(
      "sequel",
    );
  ...
  it("transforms a bare script under the system narrator", () => {
    ...
    expect(result.segments.map(({ speakerId }) => speakerId)).toEqual([
      "narrator",
      "narrator",
    ]);
    expect(result.readableTranscript).toBe(
  ...
  it("validates named-sense fields and ignores empty replacements", () => {
    ...
    const result = transform("SQL", [
      entry({ id: "empty", displayText: "SQL", spokenText: "   " }),
    ]);
