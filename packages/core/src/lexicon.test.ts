...
describe("lexicon JSON authoring", () => {
  it("applies authoring defaults while preserving required behavior", () => {
    expect(
      LexiconEntryAuthoringSchema.parse({
        scope: "global",
        entryType: "exactTerm",
        displayText: "SQL",
        spokenText: "sequel",
  ...
  it("generates deterministic neutral IDs and preserves supplied opaque IDs", () => {
    ...
    expect(
      result.entries.map(({ id, displayText }) => [id, displayText]),
    ).toEqual([
  ...
  it("accepts empty collections and empty spoken text", () => {
    expect(normalizeLexiconEntries([], { nextId: 4, timestamp: now })).toEqual({
      entries: [],
      nextId: 4,
    });
    expect(
