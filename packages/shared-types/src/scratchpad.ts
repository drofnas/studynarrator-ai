...
export const ScratchpadPreviewInputSchema = z
  .object({
...
const ScratchpadWarningSchema = z
  .object({
...
export const ScratchpadPreviewResultSchema = z
  .object({
    ...
    readableText: z
      .string()
      .min(1)
      .max(SCRATCHPAD_MAX_CHARACTERS * 2),
    transformedText: z
      .string()
      .min(1)
      .max(SCRATCHPAD_MAX_CHARACTERS * 2),
  ...
  .strict();
export type ScratchpadPreviewResult = z.infer<
  typeof ScratchpadPreviewResultSchema
>;

export interface ScratchpadClient {
  preview(
    input: ScratchpadPreviewInput,
    signal?: AbortSignal,
  ): Promise<ScratchpadPreviewResult>;
}
