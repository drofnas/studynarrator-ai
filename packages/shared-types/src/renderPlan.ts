...
const SnapshotVersionsSchema = z
  .object({
...
const RenderSpeechChunkSchema = z
  .object({
    ordinal: z.number().int().positive(),
    text: z.string().min(1),
    cacheKey: HashSchema,
...
export const RenderPlanSchema = z
  ...
  .superRefine((plan, context) => {
    plan.entries.forEach((entry, index) => {
      if (
        entry.type === "pause" &&
        (entry.durationMs === 0) !== (entry.silence === null)
      ) {
        context.addIssue({
          code: "custom",
          message:
            "Zero-duration pauses must omit silence; audible pauses must reference it.",
          path: ["entries", index, "silence"],
        });
      }
    });
  });
