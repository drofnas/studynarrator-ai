...
export const PROJECT_PREVIEW_CHANNELS = Object.freeze({
...
export const PreviewAudioSchema = z
  .object({
  ...
  .superRefine((audio, context) => {
    const padding = audio.base64.endsWith("==")
      ? 2
      : audio.base64.endsWith("=")
        ? 1
        : 0;
    ...
    if (decodedLength !== audio.byteLength) {
      context.addIssue({
        code: "custom",
        message: "Audio byte length does not match its encoded data.",
        path: ["byteLength"],
      });
    }
  });

export const PreviewCacheMetadataSchema = z
  .object({
...
const ProjectSegmentPreviewInputSchema = z
  .object({
...
const PronunciationPreviewInputSchema = z
  .object({
    mode: z.literal("pronunciation"),
    text: z
      .string()
      .max(MAX_PRONUNCIATION_PREVIEW_CHARACTERS)
      .refine(
        (value) => value.trim().length > 0,
        "Enter a pronunciation sample.",
      ),
...
export const ProjectPreviewResultSchema = z
  .object({
...
export const SpeechCacheKeyInputSchema = z
  ...
  .strict();
export const SpeechCacheProjectInputSchema = z
