...
describe("connection contracts", () => {
  ...
  it("requires one result for every progressive diagnostic stage", () => {
    expect(
      ConnectionTestSummarySchema.parse({
        schemaVersion: 1,
        overall: "connected",
  ...
  it("rejects duplicate or credential-shaped catalog content", () => {
    ...
    ).toMatchObject({ voiceId: "af_heart", favorite: false, language: null });
    expect(() =>
      VoiceCatalogSchema.parse({
        schemaVersion: 1,
        modelId: "model",
        entries: [
          { voiceId: "same", label: "One" },
          { voiceId: "same", label: "Two" },
        ],
      }),
        ...
        models: [
          {
            modelId: "model",
            voices: [
              { voiceId: "same", name: null, language: null, gender: null },
              { voiceId: "same", name: null, language: null, gender: null },
            ],
          },
        ],
