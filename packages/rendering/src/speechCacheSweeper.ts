...
export function createSpeechCacheSweeper(options: {
  ...
  return {
    async sweep(optionsInput: {
      ...
      if ((await options.cache.status()).inFlight > 0)
      ...
      try {
        ...
        if ((error as NodeJS.ErrnoException).code === "ENOENT")
          return { entriesRemoved: 0, bytesFreed: 0, skipped: false, preview };
        throw error;
      }
      ...
      for (const entry of retained.sort((left, right) => {
        if (left.metadata.status !== "ok" || right.metadata.status !== "ok")
          return 0;
        return (
          left.metadata.metadata.lastUsedAt.localeCompare(
            right.metadata.metadata.lastUsedAt,
