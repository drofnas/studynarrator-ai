...
- **D1 — resolved 2026-09-15:** The owner selected the existing FFmpeg writer and removal of the unused `node-id3` wrapper/dependency. This supersedes R24's earlier ID3-package requirement. Exact requested tags and safe rename behavior remain required.
...
| R24                               | Accepted simplification  | R24: correct MP3 tags and creation-year preservation; FFmpeg reuse and dependency removal approved by D1             |
...
All ten Ponytail findings are represented: historical-doc deletion, native npm inventory, no unified contract-map rewrite, proportionate Docker gates, approved FFmpeg reuse, removal of LAN scope, no standalone column migration, shorter contribution guides, one speech baseline, and deduplicated verification. No deletion quota or architecture rewrite is added.
...
| R05  | Task  | Setup and product identity    | P1       | Complete       | —                                      | —         |
...
| R10a | Task  | Docker scanner                | P1       | Complete       | —                                      | R10b      |
| R10b | Task  | Docker CI                     | P1       | Complete       | R10a, R07a                             | —         |
| R11  | Task  | Contributor documentation     | P1       | Complete       | —                                      | —         |
| R17  | Task  | Dependency maintenance        | P2       | Complete       | R07a                                   | —         |
| R22  | Story | Script editor                 | P1       | Complete       | —                                      | —         |
| R24  | Task  | MP3 rendering                 | P1       | Complete       | —                                      | R12b      |
| R25a | Task  | Storage service and contracts | P1       | Complete       | —                                      | R25b      |
| R25b | Story | General settings              | P1       | Complete       | R25a                                   | —         |
| R26  | Task  | Lexicon and persistence       | P1       | Complete       | —                                      | —         |
...
The remaining non-issue blocker is eventual R19 release authorization/native-platform access; D1 is resolved. Do not create an issue pretending to represent owner consent. If deferred items are omitted from initial publication, publish their native dependencies when those items are later created; do not treat absent links as evidence of readiness.
...
1. D1 was resolved on September 15; use the accepted FFmpeg scope for R24.
2. If repository-issue creation is authorized, recheck duplicates and create only remaining accepted records from TASKS/FUTURE_WORK. Preserve the original R-identifiers and accepted source amendments in each body. Deferred records can stay in FUTURE_WORK until explicitly included; no issue should imply they gate the local beta.
...
The tracker publication plan remains a review draft. R24's tagging mechanism is settled; the open publication decisions are whether to create repository issues and whether deferred records should also be published. The audit's dormant/canceled proposals never become implementation tickets.
