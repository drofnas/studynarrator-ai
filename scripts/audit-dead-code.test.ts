...
function runAudit(root: string) {
  return spawnSync(
    process.execPath,
    [resolve("scripts/audit-dead-code.mjs"), "--root", root],
    {
      cwd: resolve("."),
      encoding: "utf8",
    },
...
describe("dead-code audit", () => {
  ...
  test("finds unused files, exports, and dependencies while preserving special entrypoints", () => {
    ...
    try {
      ...
      writeFileSync(
        join(root, "src", "workers", "parser.worker.ts"),
        "self.onmessage = () => undefined;\n",
      );
      writeFileSync(
        join(root, "src", "contracts.d.ts"),
        "declare const fixtureAmbient: string;\n",
      );
      writeFileSync(
        join(root, "src", "serviceManifest.ts"),
      ...
      expect(result.stderr).toContain(
        "package.json: unused-dependency: left-pad",
      );
      expect(result.stderr).not.toMatch(
        /parser\.worker|contracts\.d\.ts|serviceManifest|migrate\.ts|smoke\.ts|styles\.css/u,
      );
    } finally {
